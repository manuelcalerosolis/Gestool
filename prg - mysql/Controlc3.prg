...
CLASS TControl FROM TWindow
   ...
   METHOD GetNewId() INLINE If( ::nInitId == nil, ::nInitId := 100,),;
                            ++::nInitId  // XBPP change
...
METHOD LButtonDown( nRow, nCol, nKeyFlags ) CLASS TControl
   ...
   if ::lDrag
      ...
      if ! ::lCaptured
         // SetFocus( ::hWnd )
         ::nPoint = nMakeLong(  nRow -= Mod( nRow, nYGrid ),  nCol -= Mod( nCol, nXGrid ) )
         nMRow  = nRow
         nMCol  = nCol
   ...
   if ::lDrag
      if ::lCaptured
         ...
         if Empty( ::nMResize )
            ...
            nRow := aPoint[ 1 ] - Mod( aPoint[ 1 ], nYGrid )
            nCol := aPoint[ 2 ] - Mod( aPoint[ 2 ], nXGrid )
            do case
