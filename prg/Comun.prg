...
FUNCTION CreateMainSQLWindow()
   ...
   oMsgDelegacion             := TMsgItem():New( oWnd:oMsgBar, "Delegaci�n : " + rtrim( Application():codigoDelegacion() ), 200,,,, .t., {|| EnviromentController():Activate() } )

   oMsgCaja                   := TMsgItem():New( oWnd:oMsgBar, "Caja : "  +      rtrim( Box():Nombre() ), 200,,,, .t., {|| EnviromentController():Activate() } )

   oMsgAlmacen                := TMsgItem():New( oWnd:oMsgBar, "Almac�n : " +    rtrim( Application():codigoAlmacen() ), 200,,,, .t., {|| EnviromentController():Activate() } )

   oMsgSesion                 := TMsgItem():New( oWnd:oMsgBar, "Sesi�n : " +     alltrim( str( Session():Numero() ) ) , 100,,,, .t., {|| EnviromentController():Activate() } )
...
FUNCTION setSesionMessageBar( nNumero )
   ...
   if !empty( nNumero ) .and. oMsgCaja != nil
      oMsgSesion:SetText( "Sesi�n : " + alltrim( str( nNumero ) ) )
   end if
...
FUNCTION setAlmacenMessageBar( cAlmacen )
   ...
   if cAlmacen != nil .and. oMsgAlmacen != nil
      oMsgAlmacen:SetText( "Almac�n : " + rtrim( cAlmacen ) )
   end if
...
FUNCTION setDelegacionMessageBar( cDelegacion )
   ...
   if cDelegacion != nil .and. oMsgDelegacion != nil
      oMsgDelegacion:SetText( "Delegaci�n : " + rtrim( cDelegacion ) )
   end if
...
FUNCTION CreateMainSQLAcceso()
   ...
   oItem                := oItemArchivo:Add()
   oItem:oGroup         := oGrupo
   oItem:cPrompt        := 'Categorias de art�culo'
   oItem:cMessage       := 'Categorias de art�culo'
   ...
   oItem                := oItemArchivo:Add()
   oItem:oGroup         := oGrupo
   oItem:cPrompt        := 'Unidades de medici�n'
   ...
   oItem                := oItemArchivo:Add()
   oItem:oGroup         := oGrupo
   oItem:cPrompt        := 'Grupos de unidades de medici�n'
   oItem:cMessage       := 'Grupos de unidades de medici�n'
   ...
   oItem                := oItemArchivo:Add()
   oItem:oGroup         := oGrupo
   oItem:cPrompt        := 'Tipos de art�culo'
   oItem:cMessage       := 'Tipos de art�culo'
   ...
   oItem                := oItemArchivo:Add()
   oItem:oGroup         := oGrupo
   oItem:cPrompt        := 'Temporadas de art�culo'
   oItem:cMessage       := 'Temporadas de art�culo'
   ...
   oItem:cMessage       := 'Albaranes'
   oItem:bAction        := {|| AlbaranesComprasController():New():ActivateNavigatorView() }
   oItem:cId            := "albaranes_compras"
   oItem:cBmp           := "gc_albaran_proveedor_16"
   oItem:cBmpBig        := "gc_albaran_proveedor_32"
   oItem:lShow          := .t.

   oItem                := oItemCompras:Add()
   oItem:oGroup         := oGrupo
   oItem:cPrompt        := 'Facturas'
   oItem:cMessage       := 'Facturas'
   ...
   /*oItem                := oItemCompras:Add()
   oItem:oGroup         := oGrupo
   oItem:cPrompt        := 'Presupuestos'
   oItem:cMessage       := 'Presupuestos'
   oItem:bAction        := {|| PresupuestosComprasController():New():ActivateNavigatorView() }
   oItem:cId            := "presupuestos_compras"
   oItem:cBmp           := "gc_document_text_businessman_16"
   oItem:cBmpBig        := "gc_document_text_businessman_32"
   oItem:lShow          := .t.*/
   ...
   oItem:cPrompt        := 'Presupuestos'
   oItem:cMessage       := 'Presupuestos'
   oItem:bAction        := {|| PresupuestosVentasController():New():ActivateNavigatorView() }
   // oItem:bAction        := {|| FacturasVentasController1000() }
   oItem:cId            := "presupuestos_ventas"
...
FUNCTION lInitCheck( oMessage, oProgress )
   ...
   // Selecci�n de la empresa actual------------------------------------------
   ...
   // Selecci�n de los datos de la aplicacion----------------------------------

   if !empty( oMessage )
      oMessage:SetText( 'Selecci�n de datos de la aplicaci�n' )
   end if
