...
function TestRTF5()
   ...
   //? HexToNum( "0602" ), NumToHex( 21249 )
   hDLL  := LoadLibrary( "Msftedit.dll" )
   ...
   lUpp  := .F.
   DEFINE FONT oFnt NAME "Segoe UI Symbol" SIZE 0, -14
   DEFINE WINDOW oWndMain FROM 0, 0 TO 23, 79 ;
          TITLE "Class TRichEdit 5" ;
          MENU SetMenu() ;
          COLOR CLR_BLACK, nil ;
          MENUINFO 4 ;
          MDI
   ...
   oWndMain:SetFont( oFont := TFont():New( GetSysFont(), 0, ;
                                           If( LargeFonts(), -10, -12 ) ) )
   */
   DEFINE FONT oFont NAME "Segoe MDL2 Assets" SIZE 0, -18  //Segoe UI Symbol
...
static function SetMenu()
   ...
   MENU oMenu  2015
        ...
        MENU
            ...
            //MENUITEM "&Load as Text" RESOURCE "Open" ACTION oRTF:LoadFromRTFFile( cFile, .T. )
            SEPARATOR
            ...
            MENUITEM "Propiedades" ACTION XBrowse( REGetCharFormat5( oRTF:hWnd ) )

            MENUITEM "Last Visible" ACTION ( MsgInfo( oRTF:LastLineVisible(), Str( oRTF:GetLineCount() ) ), ;
                     MsgInfo( Str( oRTF:Pages() ) ) )
            //MENUITEM "Count Pages" //ACTION MsgInfo( oRTF:nPages( .T. ) )  // lRefresh
            MENUITEM FWString( "Numering type " )  action RTFNumering() 
            MENUITEM "Test Insert String "  ACTION oRTF:InsertRTF()
            MENUITEM "Select Line" ACTION oRTF:SelectLine()
            
            SEPARATOR
            MENUITEM "Save As HTML" ACTION oRTF:SaveAsHtml()
        ...
        MENU
            ...
            SEPARATOR
            MENUITEM "Go Bottom" RESOURCE "Preview" ;
            MESSAGE "Move At End of Control" ;
            WHEN Len( oWndMain:oWndClient:aWnd ) > 0 ;
            ACTION oRTF:GoToLine( oRTF:GetLineCount() )
        ...
        MENU
            ...
            MENUITEM "Maps Characters" ;
               ACTION ( oRTF:SetSuperInd( .T. ), oRTF:InsertRTF( "H" ), oRTF:SetSuperInd( .F. ) )
               //ACTION WaitRun( "Charmap.exe" ) RESOURCE "option16"

            SEPARATOR
            MENUITEM "File" ;
            WHEN Len( oWndMain:oWndClient:aWnd ) > 0 ;
            ACTION ( oRTF:InsertFile()   )
            
            SEPARATOR
            MENUITEM "Paragraph" ;
            WHEN Len( oWndMain:oWndClient:aWnd ) > 0 ;
            ACTION ( RESetParagraphAll5( oRTF:hWnd, 0, 4 ) )

            SEPARATOR
            MENUITEM "Table" ;
            WHEN Len( oWndMain:oWndClient:aWnd ) > 0 ;
            ACTION ( oRTF:InsertTable( 0, 0 ) )
            ...
            MENUITEM "Page Break" ;
            WHEN Len( oWndMain:oWndClient:aWnd ) > 0 ;
            ACTION ( oRTF:PageBreak() )

            SEPARATOR
            MENUITEM "Object Word" ;
            WHEN Len( oWndMain:oWndClient:aWnd ) > 0 ;
            ACTION ( oRTF:InsertObject() )
            
            MENUITEM "Object Excel" ;
            WHEN Len( oWndMain:oWndClient:aWnd ) > 0 ;
            ACTION ( oRTF:InsertObject() )
            ...
            SEPARATOR
            MENUITEM FWString( "Line Spacing" )
            MENU
               MENUITEM FWString( "1.0")  ACTION (RTSETPARAFORMAT5( oRTF:hWnd, , , , , , 1.0,  )   )
               MENUITEM FWString( "2.0")  ACTION (RTSETPARAFORMAT5( oRTF:hWnd, , , , , , 2.0,  )   )
               MENUITEM FWString( "2.5")  ACTION (RTSETPARAFORMAT5( oRTF:hWnd, , , , , , 2.5,  )   )
               MENUITEM FWString( "3.0")  ACTION (RTSETPARAFORMAT5( oRTF:hWnd, , , , , , 3.0,  )   )
               SEPARATOR
               MENUITEM FWString( "Get Paragraph Line Spacing")  ACTION MsgInfo ( "Paragraph Line Spacing : " )
            ENDMENU

            SEPARATOR
            MENUITEM "Margin Left" ACTION ( oRTF:SetMargin( 70, , , .T. ) )
            SEPARATOR
            MENUITEM "String from code"  //ACTION  InsertHF()
            MENUITEM "String from file"  //ACTION  InsertHF()
            MENUITEM "Header and Footer" ACTION  InsertHF()
        ENDMENU
...
static function SetBar()
   ...
   DEFINE BUTTONBAR oBar OF oWndMain _3D SIZE 36, 36
...
static function New()
   ...
   //oRTF:SetAutoURLDetect( .F. )   
   //DragAcceptFiles( oRTF:hWnd, .T. )
   //oRTF:oDragCursor  := CursorArrow()
   //oRTF:bDropFiles   := { | nRow, nCol, uDropInfo | ;
   //                       oRTF:SetDropFiles( nRow, nCol, uDropInfo ) }
   //oRTF:aFmtsClpPaste   := { 13, 1, 7, 15 }

   oBar   := SetChildBar( oWnd )
   //DEFINE BUTTONBAR oBarR OF oWnd _3D SIZE 26, 27
   @ oBar:nHeight, -1 BUTTONBAR oBarR OF oBar SIZE oBar:nWidth - 1, 32 
   
   @ .27, 54 COMBOBOX oZoom VAR cZoom ITEMS aZoom ;
               SIZE 56, 100 FONT oFont ;
   ...
   @ .27, 61.2 COMBOBOX oItem VAR cFont ITEMS aFonts ;
               SIZE 162, 100 FONT oFont ;
               ON CHANGE ( oRTF:SetFontName( oItem:VarGet() ), oRTF:SetFocus() ) ;
               OF oWnd:oBar
   oItem:SetColor( , CLR_WHITE )
   
   oItem:cToolTip := "Font Name"

   @ .30, 81.7 GET oSize VAR nSize OF oWnd:oBar ;
               FONT oFont SIZE 24, 26 PICTURE "99" ;
               SPINNER MIN 6 MAX 99 ;
               ON CHANGE ( oRTF:SetFontSize( oSize:VarGet() ), oRTF:SetFocus() )

   //oRuler := TRegla():New( oWnd, -1, -1, oWnd:nWidth, 20, .T., , , , oWnd:oFont, ,;
   //                 .F., CLR_BLUE, CLR_WHITE, 10, 10 )
   ...
   oWnd:bInit  := { || RefreshBar(), SetMyControls( oWnd ) }
...
static function SetMyControls( oWnd )

   //local cRTF   := ""
   local oRuler
/*   
   oRuler := TRegla():New( oWnd:oClient, -1, -1, oWnd:oClient:nWidth, 20, .T., , , , oWnd:oFont, ,;
                    .F., CLR_BLUE, CLR_WHITE, 10, 10 )
   //oWnd:SetControl( oRuler )
   oRTF:Move( 100, )
   //@  100, 0 RICHEDIT oRTF VAR cRTF OF oWnd:oClient ;
   //        FILE "TestRTF.RTF"
*/
   //RefreshBar()
...
static function RefreshChildBar( oWnd )
   ...
   local aChar    := REGetCharFormat5( oRTF:hWnd )
   local nAlign   := REGetParaFormat5( oRTF:hWnd )
   local nYOffset := aChar[ 2 ]
   ...
   oBar[ 20 ]:lPressed := lNumber := REGetNumbering5( oRTF:hWnd )
   oBar[ 21 ]:lPressed := lBullet := REGetBullet5( oRTF:hWnd )

   oBar[ 24 ]:lPressed := lLink    := aChar[ 4 ]
   oBar[ 25 ]:lPressed := lSuper   := aChar[ 10 ]
   oBar[ 26 ]:lPressed := lSub     := aChar[ 11 ]
   oBar[ 28 ]:lPressed := lUpp     := if( !Empty( AllTrim(oRTF:GetSel()) ), ;
                                            if( Upper( AllTrim(oRTF:GetSel()) ) == AllTrim(oRTF:GetSel()), .T., .F. ), .F. )
   oBar[ 29 ]:lPressed := lProtect := aChar[ 15 ]

   //oBar[ 31 ]:VarPut( aChar[ 1 ] )  // 24  // Zoom Combobox
   oBar[ 32 ]:VarPut( aChar[ 14 ] )   // 25 // cFaceName
   oBar[ 33 ]:VarPut( aChar[ 1 ] )  // 24   // Height Font

   //if !Empty( aChar[ 2 ] )
      //? aChar[ 2 ], Valtype( aChar[ 2 ] )
   //endif
   AEval( oBar, { |x| If( x:lWhen(), x:Enable(), x:Disable() ), x:Refresh() }, 8, 29 )
...
static function SetChildBar( oWnd )
   ...
   local oBar, oCursor, oClp, oBold, oItalic, oUnderline, oBullet, oNumber, oLink
   local oSub, oSuper
   local oTable, oBarUpp, oProtect, oColor

   DEFINE CURSOR oCursor HAND

   DEFINE CLIPBOARD oClp OF oWnd FORMAT TEXT

   DEFINE BUTTONBAR oBar OF oWnd _3D SIZE 32, 32 2015

/*
         return { { 1/3, nRGB( 255, 253, 222 ), nRGB( 255, 231, 151 ) }, ;
                  { 2/3, nRGB( 255, 215,  84 ), nRGB( 255, 233, 162 ) }  }
*/
   oBar:bClrGrad   := < |lInvert, oBtn|
      if oBtn != nil .and. oBtn:IsKindOf( "TBTNBMP" ) .and. oBtn:lPressed
         return { { 9/10, nRGB( 255, 215, 84 ), nRGB( 255, 233, 162 ), .T. }, ;
                  { 1/10, nRGB( 31, 31, 31 ), nRGB( 31, 31, 31 ), .T. }  }
      elseif lInvert
...
//   DEFINE BUTTON RESOURCE "Find" OF oBar GROUP ;
   DEFINE BUTTON OF oBar GROUP ;
          PROMPT if( IsWindows10(), HB_UTF8CHR( 57660 ), "R" ) CENTER ;
          FONT oFnt ;
          MESSAGE "Replace" TOOLTIP "Replace" NOBORDER ;
          ACTION ( Replace(), oRTF:SetFocus() )

   DEFINE BUTTON oLink OF oBar ; //  RESOURCE "Right" ; 
          PROMPT if( IsWindows10(), HB_UTF8CHR( 57703 ), "H" ) CENTER ;          //82
          FONT oFnt ;
          MESSAGE "Link" TOOLTIP "Link Format" NOBORDER ;
          WHEN ! oRTF:lReadOnly ;
          ACTION ( lLink := !lLink, ;
                   oLink:lPressed := lLink, ;
                   oLink:Refresh(), ;
                   oRTF:SetLink( lLink ), ; 
                   oRTF:SetFocus() )

   DEFINE BUTTON oSuper OF oBar ; //RESOURCE "Left" ; 
          PROMPT if( IsWindows10(), HB_UTF8CHR( 57733 ), "S" ) CENTER ;
          FONT oFnt ;
          MESSAGE "SuperIndice" TOOLTIP "SuperIndice" NOBORDER ;
          ACTION ( lSuper := !lSuper, ;
                   oSuper:lPressed := lSuper, ;
                   oSuper:Refresh(), ;
                   oRTF:SetSuperInd( lSuper ), ; 
                   oRTF:SetFocus() )

   DEFINE BUTTON oSub OF oBar ; // RESOURCE "Center"; 
          PROMPT if( IsWindows10(), HB_UTF8CHR( 57800 ), "s" ) CENTER ;
          FONT oFnt ;
          MESSAGE "SubIndice" TOOLTIP "SubIndice" NOBORDER ;
          ACTION ( lSub := !lSub, ;
                   oSub:lPressed := lSub, ;
                   oSub:Refresh(), ;
                   oRTF:SetSubInd( lSub ), oRTF:SetFocus() )
          
   DEFINE BUTTON oTable OF oBar;
          PROMPT if( IsWindows10(), HB_UTF8CHR( 57698 ), "T" ) CENTER ;
          FONT oFnt ;
          MESSAGE "Insert Table" TOOLTIP "Insert Table" NOBORDER ;
          ACTION ( oRTF:InsertTable( 0, 0 ) )

 DEFINE BUTTON oBarUpp OF oBar GROUP ;  //RESOURCE "RTFCAP"
          PROMPT "Uu" CENTER ;
          MESSAGE "Formats the selection to capital letters (toggle)" ;
          TOOLTIP "Upper" NOBORDER ;
          ACTION ( lUpp := ! lUpp, ;
                         oBarUpp:lPressed := lUpp, ;
                         oBarUpp:Refresh(), ;
                         if( lUpp, oRTF:MinMay(), oRTF:MayMin() ), ; // oRTF:SetUpperCaps( lUpp ), ;
                         oRTF:SetFocus() )

   DEFINE BUTTON oProtect OF oBar ; // RESOURCE "Center"; 
          PROMPT if( IsWindows10(), HB_UTF8CHR( 57649 ), "P" ) CENTER ;
          FONT oFnt ;
          MESSAGE "Protect" TOOLTIP "Protect" NOBORDER ;
          ACTION ( lProtect := !lProtect, ;
                   oProtect:lPressed := lProtect, ;
                   oProtect:Refresh(), ;
                   oRTF:SetProtect( lProtect ), oRTF:SetFocus() )

   DEFINE BUTTON oColor OF oBar ; // RESOURCE "Center"; 
          PROMPT if( IsWindows10(), HB_UTF8CHR( 57650 ), "C" ) CENTER ;
          FONT oFnt ;
          MESSAGE "Color Text" TOOLTIP "Color Text" NOBORDER ;
          ACTION ( oRTF:SetTextColor( ChooseColor( oRTF:GetTextColor() ) ) )
   ...
   REDEFINE BUTTON ID 201 ACTION ( ; //oRTF:SetFocus(), ;
         if( !lAll, ;
            if( oRTF:Find( Trim( cFind ), ( nDir == 1 ), lCase, lWord ),;
               ( oRTF:ReplaceSel( , Trim( cReplace ) ), ;
                 oRTF:Find( Trim( cFind ), ( nDir == 1 ), lCase, lWord ) ), MsgInfo( "Text not Found" ) ),;
...
Function  RTFNumering()   
   Local  nBottom   := 26
   Local  nRight    := 55
   Local  nWidth :=  Max( nRight * DLG_CHARPIX_W, 180 )
   Local  nHeight := nBottom * DLG_CHARPIX_H
   Local  oDlgNumering, oFld
   Local  nNumberStart:= oRtf:GetParaFormat()[4]
   Local  nNumber  := oRtf:GetParaFormat()[2]
   Local  nNumberStyle := oRtf:GetParaFormat()[3]
   Local oBtn,aGet[3]

   DEFINE DIALOG oDlgNumering          ;
      SIZE nWidth, nHeight   PIXEL  ;
      TITLE "Bullets and Numering"    ;
       TRANSPARENT


   @ 10,10 say   "Numbering"      OF oDlgNumering SIZE 60,10  PIXEL
   @ 20,10 say   "NumberStyle"    OF oDlgNumering SIZE 60,10  PIXEL
   @ 30,10 say   "NumberStart"    OF oDlgNumering SIZE 60,10  PIXEL

   @ 10,70  get aGet[1] var  nNumber      OF oDlgNumering SIZE 40,10  PIXEL
   @ 20,70  get aGet[1] var  nNumberStyle OF oDlgNumering SIZE 40,10  PIXEL
   @ 30,70  get aGet[1] var  nNumberStart OF oDlgNumering SIZE 40,10  PIXEL

   @ 10,155 BUTTON oBtn PROMPT "Confirm" OF oDlgNumering SIZE 60,10  PIXEL ;
                    ACTION ( ortf:SetNumering(nNumber,nNumberStyle,nNumberStart),oDlgNumering:End())

   @ 24,155 BUTTON  "Cancelar" OF oDlgNumering SIZE 60,10  PIXEL ACTION ( oDlgNumering:End() )
...
Function InsertHF()
   ...
   cText2 := "{\rtf1\ansi\deff0"    //\deflang1033"
   cText2 += "{\fonttbl{\f0\fmodern\fprq1 Tahoma;}}" //"{\fonttbl{\f0\fmodern\fprq1\fcharset0 Courier New;}}"
   cText2 += "{\header"
   cText2 += "\viewkind4\uc1\pard\f0\fs20\qc"
   cText2 += "HEADER"
   cText2 += "}"
   //cText2 += "{\footer"
   //cText2 += "\viewkind4\uc1\pard\f0\fs20\qc"
   //cText2 += "FOOTER"
   //cText2 += "}"
   cText2 += "\margl200\margr200\margt2\margb200"  //"\margl200\margr200\margt200\margb200"
   cText2 += "\viewkind4\uc1\pard\f0\fs20"   //"\viewkind4\uc1\pard\f0\fs20\sl495"
   cText2 += "000001\par"
   cText2 += "01    \line"
   cText2 += "02    UNITED STATES DISTRICT COURT\line"
   cText2 += "03    SOUTHERN DISTRICT OF NEW YORK\line"
   cText2 += "04    DOCKET No. (DAB)\line"
   cText2 += "05    --------------------------------------------x\line"
   cText2 += "06    BLAH BLAH, et al,\line"
   cText2 += "07                                 Plaintiffs,\line"
   cText2 += "08        vs\line"
   cText2 += "09    BLAH BLAH., et al,\line"
   cText2 += "10                                 Defendants\line"
   cText2 += "11    --------------------------------------------x\line"
   cText2 += "12    \line"
   cText2 += "13         EXAMINATION BEFORE TRIAL of BLAH BLAH\line"
   cText2 += "14                   New York, New York\line"
   cText2 += "15         Tuesday, December 14, 2004, 10:22 a.m.\line"
   cText2 += "16    \line"
   cText2 += "17    \line"
   /*
   cText2 += "18    \line"
