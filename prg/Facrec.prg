...
#define _NDTOESP            40      //,"N",  5, 2, "Porcentaje de descuento especial" },;
#define _CDPP               41      //,"N",  5, 2, "Porcentaje de descuento por pronto pago" },;
#define _NDPP               42      //,"N",  5, 2, "Porcentaje de descuento por pronto pago" },;
#define _CDTOUNO            43      //,"C", 25, 0, "Descripción de porcentaje de descuento personalizado"
#define _NDTOUNO            44      //,"N",  4, 1, "Porcentaje de descuento por descuento personalizado"
#define _CDTODOS            45      //,"C", 25, 0, "Descripción de porcentaje de descuento personalizado"
#define _NDTODOS            46      //,"N",  4, 1, "Porcentaje de descuento por descuento personalizado"
#define _NDTOCNT            47      //,"N",  6, 2, "Porcentaje de Descuento por pago de Contado" },;
#define _NDTORAP            48      //,"N",  6, 2, "Porcentaje de Descuento por Rappel" },;
#define _NDTOPUB            49      //,"N",  6, 2, "Porcentaje de Descuento por Publicidad" },;
#define _NDTOPGO            50      //,"N",  6, 2, "Porcentaje de Descuento por Pago Centralizado" },;
#define _NDTOPTF            51      //,"N",  7, 2, "Descuento por plataforma" },;
#define _NTIPOIVA           52      //,"N",  1, 0, "Número del tipo de " + cImp() },;
#define _NPORCIVA           53      //,"N",  4, 1, "Porcentaje de " + cImp() },;
#define _LRECARGO           54      //,"L",  1, 0, "Lógico para recargo" },;
#define _CREMITIDO          55      //,"C", 50, 0, "Campo de remitido" },;
#define _LIVAINC            56      //,"N",  1, 0, "Selección de " + cImp() },;
#define _LSNDDOC            57      //,"L",  1, 0, "Lógico para documento enviado" },;
#define _CDIVFAC            58      //,"C",  3, 0, "Codigo de la divisa" },;
#define _NVDVFAC            59      //,"N", 10, 4, "Cambio de la divisa" },;
#define _CRETPOR            60      //,"C",100, 0, "Retirado por" },;
#define _CRETMAT            61      //,"C",  8, 0, "Matricula" } }
#define _CNUMDOC            62      //,"C",  8, 0, "Matricula" } }
#define _NREGIVA            63
#define _CCODPRO            64
#define _CDOCORG            65
#define _NNUMLIQ            66     //"N",  9, 0, "Número liquidación" }                                  "",                   "", "( cDbf )"} )
#define _CSUFLIQ            67     //"C",  2, 0, "Sufijo liquidación" }                                  "",                   "", "( cDbf )"} )
#define _NIMPLIQ            68     //"N", 16, 6, "Importe liquidación" }                                 "",                   "", "( cDbf )"} )
#define _LLIQUID            69     //"L",  1, 0, "Logico de liquidado" }                                 "",                   "", "( cDbf )"} )
#define _CCODTRN            70     //"L",  1, 0, "Logico de liquidado" }                                 "",                   "", "( cDbf )"} )
#define _NKGSTRN            71     //"L",  1, 0, "Logico de liquidado" }                                 "",                   "", "( cDbf )"} )
#define _LCLOFAC            72     //"L",  1, 0, "Logico de liquidado" }                                 "",                   "", "( cDbf )"} )
#define _CABNFAC            73     //"C", 12, 0, "Número de presupuesto
#define _CANTFAC            74     //"C", 12, 0, "Número de presupuesto
#define _NPCTRET            75
#define _CCODUSR            76
#define _DFECCRE            77
#define _CTIMCRE            78
#define _CCODGRP            79
#define _LIMPRIMIDO         80      //   L      1     0
#define _DFECIMP            81      //   D      8     0
#define _CHORIMP            82      //   C      5     0
#define _CCODDLG            83
#define _CMANOBR            84
#define _CMOTREC            85      //   C     35     0
#define _CCAUREC            86      //   C     35     0
#define _CTLFCLI            87      //   C     20     0
#define _NTOTNET            88
#define _NTOTIVA            89
#define _NTOTREQ            90
#define _NTOTFAC            91
         ...
         */
         ...
         if !Empty( oInf )
            ...
            ACTIVATE REPORT oInf ;
               WHILE       ( ( ( dbfFacRecL )->cSerie + Str( ( dbfFacRecL )->nNumFac ) + ( dbfFacRecL )->cSufFac = cNumFac .and. !( dbfFacRecL )->( eof() ) ) );
               FOR         ( !( dbfFacRecL )->lImpLin ) ;
   ...
   */
   ...
   DEFINE DIALOG oDlg RESOURCE "FacCli" TITLE LblTitle( nMode ) + "facturas rectificativas"
	...
	REDEFINE BUTTON ;
      ...
      REDEFINE SAY   oSayLabels[ 7 ] ID 709 OF oFld:aDialogs[ 1 ]
...
/*Method CreateData()
   ...
   USE ( cPatSnd() + "FacRecT.DBF" ) NEW VIA ( cDriver() ) SHARED ALIAS ( cCheckArea( "FacRecT", @tmpFacRecT ) )
   SET ADSINDEX TO ( cPatSnd() + "FacRecT.CDX" ) ADDITIVE

   USE ( cPatSnd() + "FacRecL.DBF" ) NEW VIA ( cDriver() ) SHARED ALIAS ( cCheckArea( "FacRecL", @tmpFacRecL ) )
   SET ADSINDEX TO ( cPatSnd() + "FacRecL.CDX" ) ADDITIVE

   USE ( cPatSnd() + "FacRecI.DBF" ) NEW VIA ( cDriver() ) SHARED ALIAS ( cCheckArea( "FacRecI", @tmpFacRecI ) )
   ...
   for m := 1 to len( aFiles )
      ...
      if ::oSender:lUnZipData( cPatIn() + aFiles[ m, 1 ] )
         ...
         if file( cPatSnd() + "FacRecT.Dbf" ) .and.;
            ...
            USE ( cPatSnd() + "FacRecT.DBF" ) NEW VIA ( cDriver() )READONLY ALIAS ( cCheckArea( "FacRecT", @tmpFacRecT ) )
            SET ADSINDEX TO ( cPatSnd() + "FacRecT.CDX" ) ADDITIVE
            ...
            while ( tmpFacRecT )->( !eof() )
               ...
               if lValidaOperacion( ( tmpFacRecT )->dFecFac, .f. ) .and. ;
                  ...
                  if ( tmpFacRecL )->( dbSeek( ( tmpFacRecT )->cSerie + Str( ( tmpFacRecT )->nNumFac ) + ( tmpFacRecT )->cSufFac ) )
                     while ( tmpFacRecL )->cSerie + Str( ( tmpFacRecL )->nNumFac ) + ( tmpFacRecL )->cSufFac == ( tmpFacRecT )->cSerie + Str( ( tmpFacRecT )->nNumFac ) + ( tmpFacRecT )->cSufFac .and. !( tmpFacRecL )->( eof() )
                  ...
                  ::oSender:SetText( "Desestimada factura : " + ( tmpFacRecL )->cSerie + "/" + AllTrim( Str( ( tmpFacRecL )->nNumFac ) ) + "/" +  AllTrim( ( tmpFacRecL )->cSufFac ) + "; " + Dtoc( ( tmpFacRecT )->dFecFac ) + "; " + AllTrim( ( tmpFacRecT )->cCodCli ) + "; " + ( tmpFacRecT )->cNomCli )
...
FUNCTION rxFacRec( cPath, oMeter )
   ...
   */
   ...
   dbUseArea( .t., cDriver(), cPath + "FacRecS.Dbf", cCheckArea( "FacRecS", @cFacRecT ), .f. )

   if !( cFacRecT )->( neterr() )
      ( cFacRecT )->( __dbPack() )

      ( cFacRecT )->( ordCondSet( "!Deleted()", {||!Deleted()}  ) )
      ( cFacRecT )->( ordCreate( cPath + "FacRecS.Cdx", "nNumFac", "cSerFac + Str( nNumFac ) + cSufFac + Str( nNumLin )", {|| Field->cSerFac + Str( Field->nNumFac ) + Field->cSufFac + Str( Field->nNumLin ) } ) )

      ( cFacRecT )->( ordCondSet( "!Deleted()", {|| !Deleted() } ) )
      ( cFacRecT )->( ordCreate( cPath + "FacRecS.Cdx", "cRefSer", "cRef + cAlmLin + cNumSer", {|| Field->cRef + Field->cAlmLin + Field->cNumSer } ) )

      ( cFacRecT )->( ordCondSet( "!Deleted()", {|| !Deleted() } ) )
      ( cFacRecT )->( ordCreate( cPath + "FacRecS.CDX", "cNumSer", "cNumSer", {|| Field->cNumSer } ) )

      ( cFacRecT )->( ordCondSet( "!Deleted()", {|| !Deleted() } ) )
      ( cFacRecT )->( ordCreate( cPath + "FacRecS.Cdx", "iNumFac", "'14' + cSerFac + Str( nNumFac ) + Space( 1 ) + cSufFac", {|| '14' + Field->cSerFac + Str( Field->nNumFac ) + Space( 1 ) + Field->cSufFac } ) )
...
function aItmFacRec()
   ...
   aAdd( aItmFacRec, {"CSERIE"      ,"C",  1, 0, "Serie de la factura " ,                                   "Serie",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NNUMFAC"     ,"N",  9, 0, "Número de la factura" ,                                   "Numero",                  "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CSUFFAC"     ,"C",  2, 0, "Sufijo de la factura" ,                                   "Sufijo",                  "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CTURFAC"     ,"C",  6, 0, "Sesión de la factura" ,                                   "Turno",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"DFECFAC"     ,"D",  8, 0, "Fecha de la factura" ,                                    "Fecha",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CCODCLI"     ,"C", 12, 0, "Código del cliente" ,                                     "Cliente",                 "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CCODALM"     ,"C", 16, 0, "Código de almacén" ,                                      "Almacen",                 "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CCODCAJ"     ,"C",  3, 0, "Código de caja" ,                                         "Caja",                    "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CNOMCLI"     ,"C", 80, 0, "Nombre del cliente" ,                                     "NombreCliente",           "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CDIRCLI"     ,"C",100, 0, "Domicilio del cliente" ,                                  "DomicilioCliente",        "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CPOBCLI"     ,"C", 25, 0, "Población del cliente" ,                                  "PoblacionCliente",        "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CPRVCLI"     ,"C", 20, 0, "Provincia del cliente" ,                                  "",                        "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NCODPROV"    ,"N",  2, 0, "Número de provincia cliente" ,                            "ProvinciaCliente",        "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CPOSCLI"     ,"C", 15, 0, "Código postal del cliente" ,                              "CodigoPostalCliente",     "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CDNICLI"     ,"C", 30, 0, "DNI/Cif del cliente" ,                                    "DniCliente",              "", "( cDbf )"} )
   aAdd( aItmFacRec, {"lModCli"     ,"L",  1, 0, "Lógico de modificar datos del cliente" ,                  "ModificarDatosCliente",   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"LMAYOR"      ,"L",  1, 0, "Lógico de mayorista" ,                                    "",                        "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NTARIFA"     ,"N",  1, 0, "Tarifa de precio aplicada" ,                              "Tarifa",                  "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CCODAGE"     ,"C",  3, 0, "Código del agente" ,                                      "Agente",                  "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CCODRUT"     ,"C",  4, 0, "Código de la ruta" ,                                      "Ruta",                    "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CCODTAR"     ,"C",  5, 0, "Código de la tarifa" ,                                    "Tarifa",                  "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CCODOBR"     ,"C", 10, 0, "Código de la dirección" ,                                 "Direccion",               "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NPCTCOMAGE"  ,"N",  6, 2, "Porcentaje de comisión del agente" ,                      "ComisionAgente",          "", "( cDbf )"} )
   aAdd( aItmFacRec, {"LLIQUIDADA"  ,"L",  1, 0, "Lógico de la liquidación" ,                               "",                        "", "( cDbf )"} )
   aAdd( aItmFacRec, {"LCONTAB"     ,"L",  1, 0, "Lógico de la contabilización" ,                           "Contabilizada",           "", "( cDbf )"} )
   aAdd( aItmFacRec, {"DFECENT"     ,"D",  8, 0, "Fecha de entrega" ,                                       "FechaEntrega",            "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CSUFAC"      ,"C", 10, 0, "Su factura" ,                                             "SuFactura",               "", "( cDbf )"} )
   aAdd( aItmFacRec, {"LIMPALB"     ,"L",  1, 0, "Lógico si la factura se importó de albaranes" ,           "ImportadaAlbaran",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CCONDENT"    ,"C",100, 0, "Condición de entrada" ,                                   "CondicionEntrada",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"MCOMENT"     ,"M", 10, 0, "Comentarios" ,                                            "Comentarios",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"MOBSERV"     ,"M", 10, 0, "Observaciones" ,                                          "Observaciones",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CCODPAGO"    ,"C",  2, 0, "Código del tipo de pago" ,                                "Pago",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NBULTOS"     ,"N",  5, 0, "Número de bultos" ,                                       "Bultos",            "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NPORTES"     ,"N",  6, 0, "Valor del porte" ,                                        "Portes",         "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NIVAMAN"     ,"N",  6, 2, "Porcentaje de " + cImp() + " del gasto" ,                 "ImpuestoGastos",       "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NMANOBR"     ,"N", 16, 6, "Gastos" ,                                                 "Gastos",         "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CNUMFAC"     ,"C", 12, 0, "Número de factura" ,                                      "NumeroFactura",               "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NTIPOFAC"    ,"N",  1, 0, "" ,                                                       "",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CDTOESP"     ,"C", 50, 0, "Descripción de porcentaje de descuento especial" ,        "DescripcionDescuento1",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NDTOESP"     ,"N",  6, 2, "Porcentaje de descuento especial" ,                       "PorcentajeDescuento1",       "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CDPP"        ,"C", 50, 0, "Descripción de porcentaje de descuento por pronto pago",  "DescripcionDescuento2",                  "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NDPP"        ,"N",  6, 2, "Porcentaje de descuento por pronto pago" ,                "PorcentajeDescuento2",       "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CDTOUNO"     ,"C", 25, 0, "Descripción de porcentaje de descuento personalizado",    "DescripcionDescuento3",       "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NDTOUNO"     ,"N",  6, 2, "Porcentaje de descuento por descuento personalizado" ,    "PorcentajeDescuento3",       "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CDTODOS"     ,"C", 25, 0, "Descripción de porcentaje de descuento personalizado" ,   "DescripcionDescuento4",       "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NDTODOS"     ,"N",  4, 1, "Porcentaje de descuento por descuento personalizado" ,    "PorcentajeDescuento4",       "", "( cDbf )"} )
   ...
   aAdd( aItmFacRec, {"LRECARGO"    ,"L",  1, 0, "Lógico para recargo" ,                                    "RecargoEquivalencia",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CREMITIDO"   ,"C", 50, 0, "Campo de remitido" ,                                      "Remitido",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"LIVAINC"     ,"L",  1, 0, cImp() + " incluido" ,                                     "ImpuestosIncluidos",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"LSNDDOC"     ,"L",  1, 0, "Lógico para documento enviado" ,                          "Envio",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CDIVFAC"     ,"C",  3, 0, "Código de la divisa" ,                                    "Divisa",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NVDVFAC"     ,"N", 10, 4, "Cambio de la divisa" ,                                    "ValorDivisa",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CRETPOR"     ,"C",100, 0, "Retirado por" ,                                           "RetiradoPor",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CRETMAT"     ,"C", 20, 0, "Matrícula" ,                                              "Matricula",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CNUMDOC"     ,"C", 13, 0, "" ,                                                       "",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NREGIVA"     ,"N",  1, 0, "Regimen de " + cImp() ,                                   "TipoImpuesto",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CCODPRO"     ,"C",  9, 0, "Código de proyecto en contabilidad" ,                     "ProyectoContable",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CDOCORG"     ,"C", 10, 0, "Número del documento origen" ,                            "DocumentoOrigen",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NNUMLIQ"     ,"N",  9, 0, "Número liquidación",                                      "NumeroLiquidacion",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"CSUFLIQ"     ,"C",  2, 0, "Sufijo de la liquidación",                                "SufijoLiquidacion",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"NIMPLIQ"     ,"N", 16, 6, "Importe liquidación",                                     "ImporteLiquidacion",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"DFECLIQ"     ,"D",  8, 0, "Fecha liquidación",                                       "FechaLiquidacion",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cCodTrn"     ,"C",  9, 0, "Código del transportista" ,                               "Transportista",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"nKgsTrn"     ,"N", 16, 6, "TARA del transportista" ,                                 "TaraTransportista",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"lCloFac"     ,"L",  1, 0, "" ,                                                       "DocumentoCerrado",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cAbnFac"     ,"C", 12, 0, "" ,                                                       "",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cAntFac"     ,"C", 12, 0, "Factura de anticipo" ,                                    "",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"nPctRet"     ,"N",  6, 2, "Porcentaje de retención",                                 "PorcentajeRetencion",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cCodUsr"     ,"C",  3, 0, "Código de usuario",                                       "Usuario",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"dFecCre"     ,"D",  8, 0, "Fecha de creación del documento",                         "FechaCreacion",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cTimCre"     ,"C",  5, 0, "Hora de creación del documento",                          "HoraCreacion",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cCodGrp"     ,"C",  4, 0, "Código de grupo de cliente" ,                             "GrupoCliente",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"lImprimido"  ,"L",  1, 0, "Lógico de imprimido" ,                                    "Imprimido",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"dFecImp"     ,"D",  8, 0, "Última fecha de impresión" ,                              "FechaImpresion",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cHorImp"     ,"C",  5, 0, "Hora de la última impresión" ,                            "HoraImpresion",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cCodDlg"     ,"C",  2, 0, "Código delegación" ,                                      "Delegacion",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cManObr"     ,"C",250, 0, "Literal de gastos" ,                                      "LiteralGastos",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cMotRec"     ,"C",250, 0, "Motivo de la factura rectificativa",                      "MotivoRectificativa",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cCauRec"     ,"C",250, 0, "Causa de la factura rectificativa",                       "CausaRectificativa",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cTlfCli"     ,"C", 20, 0, "Teléfono del cliente" ,                                   "TelefonoCliente",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"nTotNet"     ,"N", 16, 6, "Total neto" ,                                             "TotalNeto",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"nTotIva"     ,"N", 16, 6, "Total " + cImp() ,                                        "TotalImpuesto",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"nTotReq"     ,"N", 16, 6, "Total recargo" ,                                          "TotalRecargo",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"nTotFac"     ,"N", 16, 6, "Total factura" ,                                          "TotalDocumento",                   "", "( cDbf )"} )
   ...
   aAdd( aItmFacRec, {"tFecfac"     ,"C",  9, 0, "Hora de la factura rectificativa",                        "",                   "", "( cDbf )"} )
   aAdd( aItmFacRec, {"cCtrCoste"   ,"C",  9, 0, "Codigo del Centro de coste",                              "",                   "", "( cDbf )"} )
