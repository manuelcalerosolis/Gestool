...
CLASS TDiaFPagoFac FROM TInfGen
...
METHOD Create()
   ...
   ::AddField( "cCodPgo", "C",  3, 0, {|| "@!" },                 "Cod. Pgo.",        .f., "Código forma pago",   4, .f. )
   ::AddField( "cNomPgo", "C",200, 0, {|| "@!" },                 "Forma pago",       .f., "Nombre forma pago",  20, .f. )
   ...
   ::AddTmpIndex( "cCodCli", "cCodPgo + cCodCli" )
   ::AddTmpIndex( "cCodFec", "cCodPgo + DtoS( dFecDoc )" )
   ::AddTmpIndex( "cCodNum", "cCodPgo + cNumDoc" )

   ::AddGroup( {|| ::oDbf:cCodPgo }, {|| "Forma de pago  : " + Rtrim( ::oDbf:cCodPgo ) + "-" + Rtrim( ::oDbf:cNomPgo ) }, {||"Total forma de pago..."} )

RETURN ( Self )

//---------------------------------------------------------------------------//

METHOD OpenFiles() CLASS TDiaFPagoFac
...
METHOD CloseFiles() CLASS TDiaFPagoFac
...
METHOD lResource( cFld ) CLASS TDiaFPagoFac

   if !::StdResource( "DIAFACPGO" )
      return .f.
   end if
   ...
   */
   ...
   if !::oDefFpgInf( 70, 80, 90, 100, 600 )
      return .f.
   end if
...
METHOD lGenerate() CLASS TDiaFPagoFac
   ...
   ::aHeader      := {  {|| "Fecha   : " + Dtoc( Date() ) },;
                        ...
                        {|| "Pago    : " + if( ::lAllFpg, "Todos", AllTrim( ::cFpgDes ) + " > " + AllTrim( ::cFpgHas ) ) },;
                        {|| "Estado  : " + ::cEstado },;
                        {|| "Ordenado: " + ::cOrdenado } }
...
METHOD AppendLineFacCli( aTotal, nTotPag )
   ...
   ::oDbf:cNomPgo    := oRetFld( ::oFacCliT:cCodPago, ::oDbfFpg )
...
METHOD AppendLineFacRec( aTotal, nTotPag )
   ...
   ::oDbf:cNomPgo    := oRetFld( ::oFacRecT:cCodPago, ::oDbfFpg )
