...
Static Function StartEdtBig( aTmp, aGet, oDlgTpv, oBrwDet )
   ...
   oGrupo               := TDotNetGroup():New( oCarpeta, 306, "Acciones", .f. )
      oBtnRenombrar     := TDotNetButton():New( 60, oGrupo, "gc_sticky_note_edit_32",   "Alias",            1, {|| RenombrarUbicacion( aTmp, aGet ) }, , {|| ( dbfTmpL )->( LastRec() ) > 0 }, .f., .f., .f. )
      oBoton            := TDotNetButton():New( 60, oGrupo, "Note_Add_32",              "Nuevo ticket",     2, {|| GetNuevaVenta( aTmp, aGet ) }, , , .f., .f., .f. )
      oBoton            := TDotNetButton():New( 60, oGrupo, "gc_sticky_note_delete_32", "Pendiente cobro",  3, {|| GetSalaVenta( aTmp, aGet, .t. ) }, , , .f., .f., .f. )
...
Static Function CrearInvitacion( dbfTmpL, oBrwDet )
   ...
   DEFINE DIALOG oDlg RESOURCE "INV_TCT"
      ...
      oBtnTodasLineas   := TBtnBmp():ReDefine( 100, "gc_table_selection_all_32",,,,,{|| oBtnUnaLinea:GoUp(), oBtnTodasLineas:GoDown() }, oDlg, .f., , .f. )

      oBtnUnaLinea      := TBtnBmp():ReDefine( 110, "gc_table_selection_row_32",,,,,{|| oBtnUnaLinea:GoDown(), oBtnTodasLineas:GoUp() }, oDlg, .f., , .f. )
