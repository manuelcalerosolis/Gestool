...
FUNCTION GenPedCli( nDevice, cCaption, cCodDoc, cPrinter, nCopies )
   ...
   */
   ...
   if lVisualDocumento( cCodDoc, D():Documentos( nView ) )
      ...
      private nTotPage     := nTotLPedCli( D():PedidosClientesLineas( nView ) )
      ...
      if !Empty( oInf ) .and. oInf:lCreated
         ...
         oInf:lFinish      := .f.
         oInf:lNoCancel    := .t.
         oInf:bSkip        := {|| PedCliReportSkipper( D():PedidosClientesLineas( nView ) ) }
      ...
      if !Empty( oInf )
         ...
         ACTIVATE REPORT oInf ;
            WHILE       ( ( D():PedidosClientesLineas( nView ) )->cSerPed + Str( ( D():PedidosClientesLineas( nView ) )->nNumPed ) + ( D():PedidosClientesLineas( nView ) )->cSufPed == nNumPed .and. !( D():PedidosClientesLineas( nView ) )->( Eof() ) );
            FOR         ( !( D():PedidosClientesLineas( nView ) )->lImpLin ) ;
   ...
   DEFINE SHELL oWndBrw FROM 0, 0 TO 22, 80 ;
     ...
     with object ( oWndBrw:AddXCol() )
      ...
      with object ( oWndBrw:AddXCol() )
         ...
         :cEditPicture     := cPorDiv( ( D():PedidosClientes( nView ) )->cDivPed, dbfDiv )
         :nWidth           := 80
         :nDataStrAlign    := 1
         :nHeadStrAlign    := 1
         :lHide            := .t.
      ...
      with object ( oWndBrw:AddXCol() )
         :cHeader          := "Div."
         :bEditValue       := {|| cSimDiv( if( lEuro, cDivChg(), ( D():PedidosClientes( nView ) )->cDivPed ), dbfDiv ) }
         :nWidth           := 30
         :nDataStrAlign    := 1
         :nHeadStrAlign    := 1
      end with
      ...
      with object ( oWndBrw:AddXCol() )
         ...
         :bEditValue       := {|| sTotPedCli( ( D():PedidosClientes( nView ) )->cSerPed + Str( ( D():PedidosClientes( nView ) )->nNumPed ) + ( D():PedidosClientes( nView ) )->cSufPed, D():PedidosClientes( nView ), D():PedidosClientesLineas( nView ), D():TiposIva( nView ), dbfDiv ):nTotalAgente }
         :nWidth           := 100
         :nDataStrAlign    := 1
         :nHeadStrAlign    := 1
         :cEditPicture     := cPorDiv
      ...
      with object ( oWndBrw:AddXCol() )
         ...
         :bEditValue       := {|| if( Empty( ( D():PedidosClientes( nView ) )->cCodTrn ), "", ( D():PedidosClientes( nView ) )->cCodTrn + " - " + AllTrim( RetFld( ( D():PedidosClientes( nView ) )->cCodTrn, oTrans:GetAlias(), "cNomTrn" ) ) ) }
         :nWidth           := 200
         :lHide            := .t.
      end with
   ...
   DEFINE DIALOG oDlg RESOURCE "PEDCLI" TITLE LblTitle( nMode ) + "pedidos de clientes"
      ...
      */
      ...
      REDEFINE GET aGet[ _NIVAMAN ] VAR aTmp[ _NIVAMAN ] ;
         ...
         ON CHANGE( nTotPedCli( nil, D():PedidosClientes( nView ), dbfTmpLin, D():TiposIva( nView ), dbfDiv, dbfFPago, aTmp ) );
         BITMAP   "LUPA" ;
      ...
      REDEFINE GET aGet[ _NMANOBR ] VAR aTmp[ _NMANOBR ] ;
         ...
         PICTURE  cPorDiv ;
         WHEN     ( lWhen ) ;
         VALID    ( nTotPedCli( nil, D():PedidosClientes( nView ), dbfTmpLin, D():TiposIva( nView ), dbfDiv, dbfFPago, aTmp ), .t. ) ;
         ON CHANGE( nTotPedCli( nil, D():PedidosClientes( nView ), dbfTmpLin, D():TiposIva( nView ), dbfDiv, dbfFPago, aTmp ) );
         OF       oFld:aDialogs[1]
      ...
      REDEFINE GET aGet[_CNUMPRE] VAR aTmp[_CNUMPRE];
         ...
         ON HELP  (  BrwPreCli( aGet[ _CNUMPRE ], dbfPreCliT, dbfPreCliL, D():TiposIva( nView ), dbfDiv, dbfFPago, aGet[ _LIVAINC ] ) );
			OF 		oFld:aDialogs[1]
   ...
   */
   ...
   DEFINE DIALOG oDlg RESOURCE "LFACCLI" TITLE LblTitle( nMode ) + "lineas a pedidos de clientes"
      ...
      REDEFINE GET aGet[ _CREF ] VAR cCodArt;
         ...
         BITMAP   "LUPA" ;
         ON HELP  ( BrwArticulo( aGet[ _CREF ], aGet[ _CDETALLE ], .f., .t., oBtn, aGet[ _CLOTE ], aTmp[ _CCODPR1 ], aTmp[ _CCODPR2 ], aGet[ _CVALPR1 ], aGet[ _CVALPR2 ], aGet[ _DFECCAD ] ) ) ;
			COLOR 	CLR_GET ;
         OF       oFld:aDialogs[1]
...
Static Function PrnSerie()
	...
	local oDlg
   local oFmtDoc
   local cFmtDoc     := cFormatoDocumento( ( D():PedidosClientes( nView ) )->cSerPed, "nPedCli", dbfCount )
   local oSayFmt
   local cSayFmt
   local oSerIni
   local oSerFin
   ...
   local lInvOrden   := .f.
   local oNumCop
   local nNumCop     := if( nCopiasDocumento( ( D():PedidosClientes( nView ) )->cSerPed, "nPedCli", dbfCount ) == 0, Max( Retfld( ( D():PedidosClientes( nView ) )->cCodCli, D():Clientes( nView ), "CopiasF" ), 1 ), nCopiasDocumento( ( D():PedidosClientes( nView ) )->cSerPed, "nPedCli", dbfCount ) )
   		...
   		if !lInvOrden
      		...
      		while ( D():PedidosClientes( nView ) )->cSerPed + Str( ( D():PedidosClientes( nView ) )->nNumPed ) + ( D():PedidosClientes( nView ) )->cSufPed >= cDocIni .and. ;
         		...
         		if lCopiasPre
		            ...
		            nCopyClient := if( nCopiasDocumento( ( D():PedidosClientes( nView ) )->cSerPed, "nPedCli", dbfCount ) == 0, Max( Retfld( ( D():PedidosClientes( nView ) )->cCodCli, D():Clientes( nView ), "CopiasF" ), 1 ), nCopiasDocumento( ( D():PedidosClientes( nView ) )->cSerPed, "nPedCli", dbfCount ) )
      ...
      */
      ...
      if ( D():Articulos( nView ) )->( dbSeek( ( dbfTmpLin )->cRef ) )
         ...
         */
         ...
         ( dbfTmpLin )->nPreDiv  := nRetPreArt( ( dbfTmpLin )->nTarLin, aTmpPed[ _CDIVPED ], aTmpPed[ _LIVAINC ], D():Articulos( nView ), dbfDiv, dbfKit, D():TiposIva( nView ), , , oNewImp )
...
STATIC FUNCTION DupPedido( cFecDoc )
   ...
   nNewNumPed  := nNewDoc( ( D():PedidosClientes( nView ) )->cSerPed, D():PedidosClientes( nView ), "NPEDCLI", , dbfCount )
...
Static Function ChangeTarifa( aTmp, aGet, aTmpPed )
   ...
   if !aTmp[ __LALQUILER ]
      ...
      if nPrePro == 0
         nPrePro  := nRetPreArt( aTmp[ _NTARLIN ], aTmpPed[ _CDIVPED ], aTmpPed[ _LIVAINC ], D():Articulos( nView ), dbfDiv, dbfKit, D():TiposIva( nView ), , , oNewImp )
      end if
      ...
      nPrePro := nPreAlq( aTmp[ _CREF ], aTmp[ _NTARLIN ], aTmpPed[ _LIVAINC ], D():Articulos( nView ) )
...
Method Dialog() CLASS TPedidosClientes2PedidosProveedor
   ...
   */
   ...
   DEFINE DIALOG ::oDlg RESOURCE "ASS_PEDCLI"
      ...
      ::oArticuloDesde:bValid := {|| cArticulo( ::oArticuloDesde, D():Articulos( nView ), ::oSayArticuloDesde ) }
      ::oArticuloDesde:bHelp  := {|| brwArtPed( ::oArticuloDesde, dbfTmpPedLin, dbfDiv, D():TiposIva( nView ) ) }
      ...
      ::oArticuloHasta:bValid := {|| cArticulo( ::oArticuloHasta, D():Articulos( nView ), ::oSayArticuloHasta ) }
      ::oArticuloHasta:bHelp  := {|| brwArtPed( ::oArticuloHasta, dbfTmpPedLin, dbfDiv, D():TiposIva( nView ) ) }
...
Method CreateLines() CLASS TPedidosClientes2PedidosProveedor
   ...
   if !lErrors
      ...
      while !( D():PedidosClientesLineas( nView ) )->( Eof() )

         if ( D():PedidosClientesLineas( nView ) )->cSerPed + Str( ( D():PedidosClientesLineas( nView ) )->nNumPed ) + ( D():PedidosClientesLineas( nView ) )->cSufPed == ::cSerie + Str( ::nNumero ) + ::cSufijo .and. nTotNPedCli( D():PedidosClientesLineas( nView ) ) != 0

            ( dbfTmpPedLin )->( dbAppend() )

            ( dbfTmpPedLin )->lShow    	:= .t.
            ( dbfTmpPedLin )->lSelArt  	:= .t.
            ( dbfTmpPedLin )->cRef     	:= ( D():PedidosClientesLineas( nView ) )->cRef
            ( dbfTmpPedLin )->cDetalle 	:= ( D():PedidosClientesLineas( nView ) )->cDetalle
            ( dbfTmpPedLin )->mLngDes 		:= ( D():PedidosClientesLineas( nView ) )->mLngDes
            ( dbfTmpPedLin )->cCodPrv  	:= ( D():PedidosClientesLineas( nView ) )->cCodPrv
            ( dbfTmpPedLin )->cCodPr1  	:= ( D():PedidosClientesLineas( nView ) )->cCodPr1
            ( dbfTmpPedLin )->cCodPr2  	:= ( D():PedidosClientesLineas( nView ) )->cCodPr2
            ( dbfTmpPedLin )->cValPr1  	:= ( D():PedidosClientesLineas( nView ) )->cValPr1
            ( dbfTmpPedLin )->cValPr2  	:= ( D():PedidosClientesLineas( nView ) )->cValPr2
            ( dbfTmpPedLin )->nNumUni  	:= ( D():PedidosClientesLineas( nView ) )->nUniCaja
            ( dbfTmpPedLin )->nNumCaj  	:= ( D():PedidosClientesLineas( nView ) )->nCanPed
            ( dbfTmpPedLin )->nStkFis  	:= oStock:nTotStockAct( ( D():PedidosClientesLineas( nView ) )->cRef, , , , , RetFld( ( D():PedidosClientesLineas( nView ) )->cRef, D():Articulos( nView ), "lKitArt" ), RetFld( ( D():PedidosClientesLineas( nView ) )->cRef, D():Articulos( nView ), "nKitStk" ) )
            ( dbfTmpPedLin )->nStkDis  	:= oStock:nTotStockAct( ( D():PedidosClientesLineas( nView ) )->cRef, , , , , RetFld( ( D():PedidosClientesLineas( nView ) )->cRef, D():Articulos( nView ), "lKitArt" ), RetFld( ( D():PedidosClientesLineas( nView ) )->cRef, D():Articulos( nView ), "nKitStk" ) ) - nTotReserva( ( D():PedidosClientesLineas( nView ) )->cRef )
            ( dbfTmpPedLin )->nIva     	:= ( D():PedidosClientesLineas( nView ) )->nIva
            ( dbfTmpPedLin )->nReq     	:= ( D():PedidosClientesLineas( nView ) )->nReq
            ( dbfTmpPedLin )->nDto     	:= ( D():PedidosClientesLineas( nView ) )->nDto
            ( dbfTmpPedLin )->nDtoPrm  	:= ( D():PedidosClientesLineas( nView ) )->nDtoPrm
            ( dbfTmpPedLin )->cUniDad  	:= ( D():PedidosClientesLineas( nView ) )->cUniDad
            ( dbfTmpPedLin )->lLote    	:= ( D():PedidosClientesLineas( nView ) )->lLote
            ( dbfTmpPedLin )->nLote    	:= ( D():PedidosClientesLineas( nView ) )->nLote
            ( dbfTmpPedLin )->cLote    	:= ( D():PedidosClientesLineas( nView ) )->cLote
            ( dbfTmpPedLin )->mObsLin  	:= ( D():PedidosClientesLineas( nView ) )->mObsLin
            ( dbfTmpPedLin )->cRefPrv  	:= ( D():PedidosClientesLineas( nView ) )->cRefPrv
            ( dbfTmpPedLin )->cUnidad  	:= ( D():PedidosClientesLineas( nView ) )->cUnidad
            ( dbfTmpPedLin )->nMedUno  	:= ( D():PedidosClientesLineas( nView ) )->nMedUno
            ( dbfTmpPedLin )->nMedDos  	:= ( D():PedidosClientesLineas( nView ) )->nMedDos
            ( dbfTmpPedLin )->nMedTre  	:= ( D():PedidosClientesLineas( nView ) )->nMedTre
            ( dbfTmpPedLin )->nPreDiv  	:= nCosto( ( D():PedidosClientesLineas( nView ) )->cRef, D():Articulos( nView ), dbfKit, .f., , dbfDiv )
...
Method Select() CLASS TPedidosClientes2PedidosProveedor
   ...
   while !( dbfTmpPedLin )->( eof() )
      ...
      if ( Empty( ( dbfTmpPedLin )->cCodPrv ) .or. ( ( dbfTmpPedLin )->cCodPrv >= ::cProveedorDesde .and. ( dbfTmpPedLin )->cCodPrv <= ::cProveedorHasta ) ) .and.;
         ...
         nStk  := oStock:nTotStockAct( ( dbfTmpPedLin )->cRef, , , , , RetFld( ( dbfTmpPedLin )->cRef, D():Articulos( nView ), "lKitArt" ), RetFld( ( dbfTmpPedLin )->cRef, D():Articulos( nView ), "nKitStk" ) ) - nTotReserva( ( dbfTmpPedLin)->cRef )
         ...
         do case
            ...
            case ::nStockDisponible == 3 .and. nStk < RetFld( ( dbfTmpPedLin )->cRef, D():Articulos( nView ), "nMinimo" )
   ...
   while !( dbfTmpPedLin )->( eof() )
      ...
      if ( dbfPrv )->( dbSeek( ( dbfTmpPedLin )->cCodPrv ) )
         ...
         if cLastProveedor != ( dbfTmpPedLin )->cCodPrv
            ...
            nNumeroPedido              := nNewDoc( cSeriePedido, D():PedidosProveedores( nView ), "nPedPrv", , dbfCount )
            ...
            ( D():PedidosProveedores( nView ) )->cCodPrv    := ( dbfTmpPedLin )->cCodPrv
            ( D():PedidosProveedores( nView ) )->cCodAlm    := ::cCodigoAlmacen
            ( D():PedidosProveedores( nView ) )->cCodCaj    := oUser():cCaja()
            ( D():PedidosProveedores( nView ) )->cNomPrv    := ( dbfPrv )->Titulo
            ( D():PedidosProveedores( nView ) )->cDirPrv    := ( dbfPrv )->Domicilio
            ( D():PedidosProveedores( nView ) )->cPobPrv    := ( dbfPrv )->Poblacion
            ( D():PedidosProveedores( nView ) )->cProPrv    := ( dbfPrv )->Provincia
            ( D():PedidosProveedores( nView ) )->cPosPrv    := ( dbfPrv )->cCodPai
            ( D():PedidosProveedores( nView ) )->cDniPrv    := ( dbfPrv )->Nif
            ( D():PedidosProveedores( nView ) )->dFecEnt    := GetSysDate() + ( dbfPrv )->nPlzEnt
            ( D():PedidosProveedores( nView ) )->nEstado    := 1
            ( D():PedidosProveedores( nView ) )->cDivPed    := cDivEmp()
            ( D():PedidosProveedores( nView ) )->nVdvPed    := nChgDiv( ( D():PedidosProveedores( nView ) )->cDivPed, dbfDiv )
            ( D():PedidosProveedores( nView ) )->lSndDoc    := .t.
            ( D():PedidosProveedores( nView ) )->cCodUsr    := cCurUsr()
            ( D():PedidosProveedores( nView ) )->cNumPedCli := ::cSerie + Str( ::nNumero ) + ::cSufijo
            ( D():PedidosProveedores( nView ) )->lRecargo   := ( dbfPrv )->lReq
         ...
         */
         ...
         ( D():PedidosProveedoresLineas( nView ) )->cRef             := ( dbfTmpPedLin )->cRef
         ( D():PedidosProveedoresLineas( nView ) )->cDetalle         := ( dbfTmpPedLin )->cDetalle
		   ( D():PedidosProveedoresLineas( nView ) )->mLngDes          := ( dbfTmpPedLin )->mLngDes
         ( D():PedidosProveedoresLineas( nView ) )->nIva             := ( dbfTmpPedLin )->nIva
         ( D():PedidosProveedoresLineas( nView ) )->nReq             := ( dbfTmpPedLin )->nReq
         ( D():PedidosProveedoresLineas( nView ) )->cPedCli          := ::cSerie + Str( ::nNumero ) + ::cSufijo
         ( D():PedidosProveedoresLineas( nView ) )->cAlmLin          := ::cCodigoAlmacen
         ( D():PedidosProveedoresLineas( nView ) )->nCanPed          := ( dbfTmpPedLin )->nNumCaj
         ( D():PedidosProveedoresLineas( nView ) )->nPreDiv          := ( dbfTmpPedLin )->nPreDiv
         ( D():PedidosProveedoresLineas( nView ) )->cUniDad          := ( dbfTmpPedLin )->cUniDad
         ( D():PedidosProveedoresLineas( nView ) )->nDtoLin          := ( dbfTmpPedLin )->nDto
         ( D():PedidosProveedoresLineas( nView ) )->nDtoPrm          := ( dbfTmpPedLin )->nDtoPrm
         ( D():PedidosProveedoresLineas( nView ) )->cCodPr1          := ( dbfTmpPedLin )->cCodPr1
         ( D():PedidosProveedoresLineas( nView ) )->cCodPr2          := ( dbfTmpPedLin )->cCodPr2
         ( D():PedidosProveedoresLineas( nView ) )->cValPr1          := ( dbfTmpPedLin )->cValPr1
         ( D():PedidosProveedoresLineas( nView ) )->cValPr2          := ( dbfTmpPedLin )->cValPr2
         ( D():PedidosProveedoresLineas( nView ) )->lLote            := ( dbfTmpPedLin )->lLote
         ( D():PedidosProveedoresLineas( nView ) )->nLote            := ( dbfTmpPedLin )->nLote
         ( D():PedidosProveedoresLineas( nView ) )->cLote            := ( dbfTmpPedLin )->cLote
         ( D():PedidosProveedoresLineas( nView ) )->mObsLin          := ( dbfTmpPedLin )->mObsLin
         ( D():PedidosProveedoresLineas( nView ) )->cRefPrv          := ( dbfTmpPedLin )->cRefPrv
         ( D():PedidosProveedoresLineas( nView ) )->nMedUno          := ( dbfTmpPedLin )->nMedUno
         ( D():PedidosProveedoresLineas( nView ) )->nMedDos          := ( dbfTmpPedLin )->nMedDos
         ( D():PedidosProveedoresLineas( nView ) )->nMedTre          := ( dbfTmpPedLin )->nMedTre
         ( D():PedidosProveedoresLineas( nView ) )->cUnidad          := ( dbfTmpPedLin )->cUnidad

         do case
            case ::nStockFin == 1

               ( D():PedidosProveedoresLineas( nView ) )->nUniCaja   := Calculaunidades( ( dbfTmpPedLin )->nNumUni, ( dbfTmpPedLin )->nStkDis, RetFld( ( dbfTmpPedLin )->cRef, D():Articulos( nView ), "nMinimo" ) )

            case ::nStockFin == 2

               ( D():PedidosProveedoresLineas( nView ) )->nUniCaja   := Calculaunidades( ( dbfTmpPedLin )->nNumUni, ( dbfTmpPedLin )->nStkDis, RetFld( ( dbfTmpPedLin )->cRef, D():Articulos( nView ), "nMaximo" ) )

            case ::nStockFin == 3

               ( D():PedidosProveedoresLineas( nView ) )->nUniCaja   := ( dbfTmpPedLin )->nNumUni
...
Method DestroyLines( lPrint ) CLASS TPedidosClientes2PedidosProveedor
   ...
   /*
   Impresión de pedidos de proveedor-------------------------------------------
	*/
...
Method EditLine() CLASS TPedidosClientes2PedidosProveedor
   ...
   DEFINE DIALOG ::oDlgEditLine RESOURCE "EdtPedPrv"
      ...
      ::oGetArticulo:bValid   := {|| cArticulo( ::oGetArticulo, D():Articulos( nView ), ::oGetArticulo:oHelpText ) }
...
Static Function DataReport( oFr )
   ...
   */
   ...
   oFr:SetMasterDetail( "Pedidos", "Almacen",                           {|| ( D():PedidosClientes( nView ) )->cCodAlm } )
...
Static Function RecalculaTotal( aTmp )
   ...
   local nTotPedCli  := nTotPedCli( nil, D():PedidosClientes( nView ), dbfTmpLin, D():TiposIva( nView ), dbfDiv, dbfFPago, aTmp, nil, .f. )
         ...
         if ( D():Articulos( nView ) )->( dbSeek( ( dbfKit )->cRefKit ) )
            ...
            */
            ...
            if ( dbfTmpLin )->lKitPrc
               ( dbfTmpLin )->nPreDiv  := nRetPreArt( nTarLin, aTmpPed[ _CDIVPED ], aTmpPed[ _LIVAINC ], D():Articulos( nView ), dbfDiv, dbfKit, D():TiposIva( nView ), , , oNewImp )
            end if
      ...
      */
      ...
      if ( D():Articulos( nView ) )->( dbSeek( cCodArt ) ) .or. ( D():Articulos( nView ) )->( dbSeek( Upper( cCodArt ) ) )
         ...
         */
         ...
         if ( lChgCodArt ) .or. ( cPrpArt != cOldPrpArt )
            ...
            */
            ...
            if nPrePro == 0
               aGet[_NPREDIV]:cText( nRetPreArt( aTmp[ _NTARLIN ], aTmpPed[ _CDIVPED ], aTmpPed[ _LIVAINC ], D():Articulos( nView ), dbfDiv, dbfKit, D():TiposIva( nView ), , , oNewImp ) )
            else
               aGet[_NPREDIV]:cText( nPrePro )
            end if
         ...
         */
         ...
         lBuscaOferta( aTmp[ _CREF ], aGet, aTmp, aTmpPed, dbfOferta, dbfDiv, dbfKit, D():TiposIva( nView )  )
...
Static Function StartPedidosWeb( oDlgPedidosWeb )
   ...
   oBoton                     := TDotNetButton():New( 60, oGrupo, "Garbage_Empty_32",  "Eliminar",       2, {|| WinDelRec( oBrwPedidosWeb, D():PedidosClientes( nView ), {|| QuiPedCli() } ), ChangePedidosWeb() } , , , .f., .f., .f. )
...
static function lBuscaOferta( cCodArt, aGet, aTmp, aTmpPed, dbfOferta, dbfDiv, dbfKit, dbfIva  )
   ...
   if ( D():Articulos( nView ) )->Codigo == cCodArt .or. ( D():Articulos( nView ) )->( dbSeek( cCodArt ) )
      ...
      */
      ...
      sOfeArt     := sOfertaArticulo( cCodArt, aTmpPed[ _CCODCLI ], aTmpPed[ _CCODGRP ], aTmp[ _NUNICAJA ], dFecPed, dbfOferta, aTmp[ _NTARLIN ], , aTmp[_CCODPR1], aTmp[_CCODPR2], aTmp[_CVALPR1], aTmp[_CVALPR2], aTmp[ _CDIVPED ], D():Articulos( nView ), dbfDiv, dbfKit, D():TiposIva( nView ), aTmp[ _NCANPED ], nTotalLinea )
      ...
      if !Empty( hAtipica )
         ...
         if hhaskey( hAtipica, "nImporte" )
            if hAtipica[ "nImporte" ] != 0
            	...
            	nRetPreArt( ( dbfTmpLin )->nTarLin, aTmpPed[ _CDIVPED ], aTmpPed[ _LIVAINC ], D():Articulos( nView ), dbfDiv, dbfKit, D():TiposIva( nView ), , , oNewImp )
            end if
         end if
...
FUNCTION aTotPedCli( cPed, cPedCliT, cPedCliL, cdbfIva, dbfDiv, dbfFPago, cDivRet )

   nTotPedCli( cPed, cPedCliT, cPedCliL, cdbfIva, dbfDiv, dbfFPago, nil, cDivRet, .f. )

RETURN ( { nTotNet, nTotIva, nTotReq, nTotPed, nTotPnt, nTotTrn, nTotAge, nTotCos } )

//--------------------------------------------------------------------------//

FUNCTION BrwPedCli( oGet, cPedCliT, cPedCliL, cdbfIva, dbfDiv, dbfFPago, oIva )
   ...
   while ! oFilEdm:lEoF()
      ...
      */
      ...
      if ( D():Clientes( nView ) )->( dbSeek( cCodCli ) )
         ...
         nNumPed                    := nNewDoc( ( D():Clientes( nView ) )->Serie, D():PedidosClientes( nView ), "NPEDCLI", , dbfCount )
         ...
         ( D():PedidosClientes( nView ) )->nVdvPed    := nChgDiv( ( D():PedidosClientes( nView ) )->cDivPed, dbfDiv )
         ( D():PedidosClientes( nView ) )->nEstado    := 1
    ...
    while !( dbfPedCliT )->( eof() )
        ...
        */
        ...
        if ( dbfPedCliT )->nTotPed == 0 .and. dbLock( dbfPedCliT )
           	...
           	aTotPed                 := aTotPedCli( ( dbfPedCliT )->cSerPed + Str( ( dbfPedCliT )->nNumPed ) + ( dbfPedCliT )->cSufPed, dbfPedCliT, dbfPedCliL, cdbfIva, dbfDiv, dbfFPago, ( dbfPedCliT )->cDivPed )
...
FUNCTION BrwArtPed( aGet, dbfTmpPedLin, dbfDiv, cdbfIva )
...
function aItmPedCli()
   ...
   aAdd( aItmPedCli, { "DFECENTR","D",    8,  0, "Fecha de entrada de alquiler",                            "EntradaAlquiler",         "", "( cDbf )", nil } )
   aAdd( aItmPedCli, { "DFECSAL", "D",    8,  0, "Fecha de salida de alquiler",                             "SalidaAlquiler",          "", "( cDbf )", nil } )
...
function aColPedCli()
   ...
   aAdd( aColPedCli, { "cValPr1",   "C",   40,  0, "Valor de la primera propiedad",                   "ValorPropiedad1",         "", "( cDbfCol )", nil } )
   ...
   aAdd( aColPedCli, { "CALMLIN",   "C",   16,  0, "Código de almacén" ,                              "Almacen",                 "", "( cDbfCol )", nil } )
   ...
   aAdd( aColPedCli, { "CLOTE",     "C",   12,  0, "Número de lote",                                  "Lote",                    "", "( cDbfCol )", nil } )
   ...
   aAdd( aColPedCli, { "cCodTip",   "C",    3,  0, "Código del tipo de artículo",                     "TipoArticulo",            "", "( cDbfCol )", nil } )
   ...
   aAdd( aColPedCli, { "CIMAGEN",   "C",  250,  0, "Fichero de imagen" ,                              "Imagen",                  "", "( cDbfCol )", .t. } )
   ...
   aAdd( aColPedCli, { "DFECENT" ,  "D",    8,  0, "Fecha de entrada del alquiler",                   "FechaEntradaAlquiler",    "", "( cDbfCol )", nil } )
   aAdd( aColPedCli, { "dFecSal" ,  "D",    8,  0, "Fecha de salida del alquiler",                    "FechaSalidaAlquiler",     "", "( cDbfCol )", nil } )
   ...
   aAdd( aColPedCli, { "nTarLin",   "N",    1,  0, "Tarifa de precio aplicada" ,                      "Tarifa",                  "", "( cDbfCol )", nil } )
   ...
   aAdd( aColPedCli, { "dFecUltCom","D",    8,  0, "Fecha ultima venta",                              "FechaUltinaVenta",        "", "( cDbfCol )", nil } )
...
Function MuestraPedidosWeb( oBtnPedidos, lGoPedCli )
   ...
   BEGIN SEQUENCE
      ...
      */
      ...
      with object ( oBrwPedidosWeb:AddCol() )
         :cHeader          := "Importe"
         :bEditValue       := {|| nTotPedCli( ( dbfPedCliT )->cSerPed + Str( ( dbfPedCliT )->nNumPed ) + ( dbfPedCliT )->cSufPed, dbfPedCliT, dbfPedCliL, cdbfIva, dbfDiv, dbfFPago, nil, cDivEmp(), .t. ) }
         :nWidth           := 80
         :nDataStrAlign    := 1
         :nHeadStrAlign    := 1
      end with
...
Function sTotPedCli( cPedido, dbfMaster, dbfLine, cdbfIva, dbfDiv, cDivRet )

   local sTotal

   nTotPedCli( cPedido, dbfMaster, dbfLine, cdbfIva, dbfDiv, nil, nil, cDivRet, .f. )
