...
#define _CTIMCRE            81
#define _CCODGRP            82
#define _LIMPRIMIDO         83      //   L      1     0
#define _DFECIMP            84      //   D      8     0
#define _CHORIMP            85      //   C      5     0
#define _CCODDLG            86
#define _NDTOATP            87      //   N      6     2     Porcentaje de descuento atípico
#define _NSBRATP            88      //   N      1     0     Lugar donde aplicar dto atípico
#define _DFECENTR           89
#define _DFECSAL            90
#define _LALQUILER          91
#define _LPAYCLI            92
#define _NPAYCLI            93
#define _CMANOBR            94
#define _LEXPEDI            95
...
static bEdtPda             := { |aTmp, aGet, dbfFacCliT, oBrw, bWhen, bValid, nMode         | EdtPda( aTmp, aGet, dbfFacCliT, oBrw, bWhen, bValid, nMode ) }
static bDetPda             := { |aTmp, aGet, dbfFacCliL, oBrw, bWhen, bValid, nMode, aTmpFac| DetPda( aTmp, aGet, dbfFacCliL, oBrw, bWhen, bValid, nMode, aTmpFac ) }
static bIncPda             := { |aTmp, aGet, dbfFacCliI, oBrw, bWhen, bValid, nMode, aTmpLin| IncPda( aTmp, aGet, dbfFacCliI, oBrw, bWhen, bValid, nMode, aTmpLin ) }
static bPgoPda             := { |aTmp, aGet, dbfFacCliP, oBrw, bWhen, bValid, nMode, aTmpFac| PgoPda( aTmp, aGet, dbfFacCliP, oBrw, bWhen, bValid, nMode, aTmpFac ) }
...
STATIC FUNCTION OpenFiles( lExt )
   ...
   BEGIN SEQUENCE
      ...
      USE ( cPatEmp() + "FACCLIA.DBF" ) NEW VIA ( cDriver() ) SHARED ALIAS ( cCheckArea( "FACCLIA", @dbfFacCliA ) )
      SET ADSINDEX TO ( cPatEmp() + "FACCLIA.CDX" ) ADDITIVE
   ...
   */
   ...
   DEFINE DIALOG oDlg RESOURCE "FACCLI" TITLE LblTitle( nMode ) + "facturas a clientes"
      ...
      */
      ...
      REDEFINE GET aGet[ _CCODCLI ] VAR aTmp[ _CCODCLI ] ;
         ID       170 ;
         WHEN     ( lWhen ) ;
         VALID    ( loaCli( aGet, aTmp, nMode, oRieCli ) );
         BITMAP   "LUPA" ;
      ...
      */
      ...
      REDEFINE BUTTON oBtnExp;
         ID       600 ;
			OF 		oFld:aDialogs[1] ;
         ACTION   ( ShowExp( oBtnExp, oBrwLin, oSayLabels, aGet, .t., oGetDtoEnt, oGetEnt ) )
      ...
      */
      ...
      oBrwLin:nClrText        := {|| if( ( dbfTmpLin )->lKitChl, CLR_GRAY, CLR_BLACK ) }
      ...
      with object ( oBrwLin:AddCol() )
         :cHeader             := "Precio"
         :bEditValue          := {|| nImpUFacCli( dbfTmpLin, nDouDiv ) }
      ...
      with object ( oBrwLin:AddCol() )
         :cHeader             := "Total"
         :bEditValue          := {|| nTotLFacCli( dbfTmpLin, nDouDiv, nRouDiv, nil, .t., .t., .t. ) }
         :cEditPicture        := cPorDiv
         :nWidth              := 80
         :nDataStrAlign       := 1
         :nHeadStrAlign       := 1
     ...
     */
     ...
     REDEFINE GET aGet[ _NPCTRET ] VAR aTmp[ _NPCTRET ] ;
      ...
      REDEFINE GET aGet[ _CNUMALB ] VAR aTmp[ _CNUMALB ] ;
			ID 		150 ;
         WHEN     ( .f. ) ;
         VALID    ( cAlbCli( aGet, aTmp, oBrwLin, nMode, oBrwPgo ), SetDialog( aGet, oSayDias, oSayGetRnt, oGetRnt ), RecalculaTotal( aTmp ) ) ;
         PICTURE  "@R #/#########/##" ;
			OF 		oFld:aDialogs[1]
      ...
      REDEFINE GET aGet[ _CNUMPED ] VAR aTmp[ _CNUMPED ] ;
         ID       151 ;
         WHEN     ( .f. ) ;
         VALID    ( cPedCli( aGet, aTmp, oBrwLin, nMode ), SetDialog( aGet, oSayDias, oSayGetRnt, oGetRnt ), RecalculaTotal( aTmp ) );
         PICTURE  "@R #/#########/##" ;
			OF 		oFld:aDialogs[1]
      ...
      */
      ...
      with object ( oBrwAnt:AddCol() )
         :cHeader             := "Fecha"
         :bEditValue          := {|| Dtoc( RetFld( ( dbfTmpAnt )->cSerAnt + Str( ( dbfTmpAnt )->nNumAnt ) + ( dbfTmpAnt )->cSufAnt, dbfAntCliT, "dFecAnt" ) ) }
         :nWidth              := 70
      end with

      with object ( oBrwAnt:AddCol() )
         :cHeader             := "Cliente"
         :bEditValue          := {|| RetFld( ( dbfTmpAnt )->cSerAnt + Str( ( dbfTmpAnt )->nNumAnt ) + ( dbfTmpAnt )->cSufAnt, dbfAntCliT, "cCodCli" ) }
         :nWidth              := 70
      end with

      with object ( oBrwAnt:AddCol() )
         :cHeader             := "Nombre"
         :bEditValue          := {|| RetFld( ( dbfTmpAnt )->cSerAnt + Str( ( dbfTmpAnt )->nNumAnt ) + ( dbfTmpAnt )->cSufAnt, dbfAntCliT, "cNomCli" ) }
         :nWidth              := 150
      end with
      ...
      with object ( oBrwAnt:AddCol() )
         :cHeader             := "Importe"
         :bEditValue          := {|| nImpAntCli( dbfTmpAnt, dbfDiv ) }
         :cEditPicture        := cPorDiv
         :nWidth              := 70
         :nDataStrAlign       := 1
         :nHeadStrAlign       := 1
      ...
      */
      ...
      REDEFINE SAY   oSayLabels[ 5 ] ID 711 OF oFld:aDialogs[ 1 ]
      REDEFINE SAY   oSayLabels[ 6 ] ID 712 OF oFld:aDialogs[ 1 ]
   ...
   ACTIVATE DIALOG oDlg ;
      ON INIT  (  EdtRecMenu( aTmp, oDlg ),;
                  SetDialog( aGet, oSayDias, oSayGetRnt, oGetRnt ),;
                  oBrwLin:Load(),;
                  oBrwInc:Load(),;
                  oBrwPgo:Load(),;
                  oBrwAnt:Load(),;
                  InitDialog( aTmp, aGet, oBtn, aNumDoc, nMode, oBrwLin ) );
...
Static Function InitDialog( aTmp, aGet, oBtn, aNumDoc, nMode, oBrwLin )
   ...
   if IsObject( aNumDoc ) .and. !Empty( aNumDoc )
      ...
      aGet[ _CCODCLI ]:cText( aNumDoc:cCodigoCliente )
      ...
      if IsNum( aNumDoc:nVentaBruta ) .and. ( aNumDoc:nVentaBruta != 0 )
         ...
         ( dbfTmpLin )->cCodAge     := aTmp[ _CCODAGE    ]
         ( dbfTmpLin )->nComAge     := aTmp[ _NPCTCOMAGE ]
         ( dbfTmpLin )->nDto        := aNumDoc:nPorcentaje
      end if
...
Static Function ShowExp( oBtnExp, oBrw, oSayLabels, aGet, lSet, oGetDtoEnt, oGetEnt )

   static lShwExp    := .f. //lShwKit()
   ...
   if lShwExp
      ...
      oBrw:Move( oBrw:nTop, oBrw:nLeft, 740, 264, .t. )
      ...
      oBrw:Move( oBrw:nTop, oBrw:nLeft, 740, 160, .t. )
...
FUNCTION nImpUFacCli( dbfTmpLin, nDec, nVdv )
   ...
   if ValType( dbfTmpLin ) == "C"

      if ( dbfTmpLin )->lAlquiler
         nCalculo    := ( dbfTmpLin )->nPreAlq
      else
         nCalculo    := ( dbfTmpLin )->nPreUnit
      end if

      if ( dbfTmpLin )->lIvaLin

         if ( dbfTmpLin )->nIva != 0
            nCalculo -= nCalculo / ( 100 / ( dbfTmpLin )->nIva + 1 )
         end if

         if ( dbfTmpLin )->nValImp != 0
            nCalculo -= ( dbfTmpLin )->nValImp
         end if
...
FUNCTION nBrtLFacCli( uTmpLin, nDec, nRec, nVdv, cPorDiv )
   ...
   nCalculo          := nImpUFacCli( uTmpLin, nDec, nVdv, cPorDiv )
   ...
   */
   ...
   if !aTmp[ __LALQUILER ]
      ...
      REDEFINE GET aGet[ _NIMPTRN ] VAR aTmp[ _NIMPTRN ] ;
         ...
         ON CHANGE( lCalcDeta( aTmp ) );
         VALID    ( lCalcDeta( aTmp ) );
         PICTURE  cPouDiv ;
         OF       oFld:aDialogs[1]
   ...
   if aTmp[ __LALQUILER ]
      ...
      REDEFINE GET aGet[_NVALIMP] VAR aTmp[_NVALIMP] ;
		...
		REDEFINE GET aGet[_NCANENT] VAR aTmp[_NCANENT] ;
         ...
         ON CHANGE( lCalcDeta( aTmp ), loaArt( aGet, bmpImage, aTmp, aTmpFac, oStkAct, oSayPr1, oSayPr2, oSayVp1, oSayVp2, nMode, .f. ) );
         VALID    ( lCalcDeta( aTmp ), loaArt( aGet, bmpImage, aTmp, aTmpFac, oStkAct, oSayPr1, oSayPr2, oSayVp1, oSayVp2, nMode, .f. ) );
			PICTURE 	cPicUnd ;
         OF       oFld:aDialogs[1] ;
         IDSAY    131
...
if ( IsMuebles() )
      ...
      REDEFINE BTNBMP ;
         ...
         ACTION   ( DesgPnt( cCodArt, aTmp, aTmp[ _NTARLIN ], aGet[ _NPREUNIT ], aGet[ _NCOSDIV ], nMode ), lCalcDeta( aTmp ) )
      ...
      REDEFINE GET aGet[ _NTARLIN ] VAR aTmp[ _NTARLIN ];
         ...
         WHEN     ( nMode != ZOOM_MODE .and. ( lUsrMaster() .or. oUser():lCambiarPrecio() ) );
         ON CHANGE( ChangeTarifa( aTmp, aGet, aTmpFac ), lCalcDeta( aTmp ) );
         OF       oFld:aDialogs[1]


if ( IsMuebles() )
      aGet[ _NPREUNIT ]:bHelp := {|| DesgPnt( cCodArt, aTmp, aTmp[ _NTARLIN ], aGet[ _NPREUNIT ], aGet[ _NCOSDIV ], nMode ), lCalcDeta( aTmp ) }
end if
...
STATIC FUNCTION cAlbCli( aGet, aTmp, oBrw, nMode, oBrwPgo )
...
static function QuiFacCli()
   ...
   */
   ...
   /*
   Eliminamos los anticipos----------------------------------------------------
   */

   nOrdAnt     := ( dbfFacCliA )->( OrdSetFocus( "nNumFac" ) )

   while ( dbfFacCliA )->( dbSeek( cSerDoc + Str( nNumDoc ) + cSufDoc ) ) .and. !( dbfFacCliA )->( eof() )

      if dbLock( dbfFacCliA )
         ( dbfFacCliA )->( dbDelete() )
         ( dbfFacCliA )->( dbUnLock() )
      end if
   ...
   ( dbfFacCliA )->( OrdSetFocus( nOrdAnt ) )
...
STATIC FUNCTION cPedCli( aGet, aTmp, oBrw, nMode )
         ...
         if ( dbfPedCliL )->( dbSeek( cPedido ) )
            ...
            while ( ( dbfPedCliL )->cSerPed + Str( ( dbfPedCliL )->nNumPed ) + ( dbfPedCliL )->cSufPed == cPedido )
               ...
               */
               ...
               //nTotRet              := ( NotCaja( ( dbfPedCliL )->nCanPed ) * ( dbfPedCliL )->nUniCaja ) - ( dbfPedCliL )->nUniEnt
...
STATIC FUNCTION GrpAlb( aGet, aTmp, oBrw )
   ...
   */
   ...
   DEFINE DIALOG  oDlg ;
      ...
      TITLE       "Cliente : " + RTrim( aTmp[_CNOMCLI] ) + Space(1) + "Obra : " + If( Empty( aTmp[_CCODOBR] ), "TODAS", aTmp[_CCODOBR] ) + Space(1) + "Albaranes: " + if( aTmp[ _LIVAINC ], "IGIC incluido", "IGIC desglosado" )
      ...
      with object ( oBrwDet:AddCol() )
         :cHeader          := "Número"
         :bEditValue       := {|| aAlbaranes[ oBrwDet:nArrayAt, 2 ] }
            ...
            while ( tmpFacCliT )->( !eof() )
               ...
               */
               ...
               if lValidaOperacion( ( tmpFacCliT )->dFecFac, .f. ) .and. ;
                  ...
                  /*
                  oStock:FacCli( ( dbfFacCliT )->cSerie + Str( ( dbfFacCliT )->nNumFac ) + ( dbfFacCliT )->cSufFac, ( dbfFacCliT )->cCodAlm, .f., .f. )
                  */
...
function SynFacCli( cPath )
   ...
   BEGIN SEQUENCE
      ...
      USE ( cPath + "FacCliA.Dbf" ) NEW VIA ( cDriver() ) ALIAS ( cCheckArea( "FacCliA", @dbfFacCliA ) ) EXCLUSIVE
      SET ADSINDEX TO ( cPath + "FacCliA.Cdx" ) ADDITIVE
...
Static Function DataReport( oFr )
   ...
   */
   ...
   oFr:SetWorkArea(     "Anticipos de facturas", ( dbfFacCliA )->( Select() ) )
   oFr:SetFieldAliases( "Anticipos de facturas", cItemsToReport( aAntFacCli() ) )
...
Static Function DetPda( aTmp, aGet, dbfFacCliL, oBrw, lTotLin, cCodArtEnt, nMode, aTmpFac )
   ...
   */
   ...
   ACTIVATE DIALOG oDlg CENTER ON PAINT ( lCalcDeta( aTmp ) )
...
FUNCTION mkFacCli( cPath, oMeter, lReindex )
   ...
   if !lExistTable( cPath + "FacCliA.Dbf" )
      dbCreate( cPath + "FacCliA.Dbf", aSqlStruct( aAntFacCli() ), cDriver() )
   end if
...
FUNCTION rxFacCli( cPath, oMeter )
   ...
   */
   ...
   dbUseArea( .t., cDriver(), cPath + "FacCliA.Dbf", cCheckArea( "FacCliA", @dbfFacCliT ), .f. )
   ...
   if !( dbfFacCliT )->( neterr() )
      ...
      ( dbfFacCliT )->( ordCondSet( "!Deleted()", {||!Deleted()}  ) )
      ( dbfFacCliT )->( ordCreate( cPath + "FacCliA.Cdx", "nNumFac", "cSerFac + Str( nNumFac ) + cSufFac", {|| Field->cSerFac + Str( Field->nNumFac ) + Field->cSufFac } ) )

      ( dbfFacCliT )->( ordCondSet( "!Deleted()", {|| !Deleted() } ) )
      ( dbfFacCliT )->( ordCreate( cPath + "FacCliA.Cdx", "nNumAnt", "cSerAnt + Str( nNumAnt ) + cSufAnt", {|| Field->cSerAnt + Str( Field->nNumAnt ) + Field->cSufAnt } ) )

      ( dbfFacCliT )->( dbCloseArea() )

   else

      msgStop( "Imposible abrir en modo exclusivo la tabla de anticipos de facturas de clientes" )
...
function aItmFacCli()
   ...
   aAdd( aItmFacCli, {"dFecSal"     ,"D",  8,  0, "fecha de salidad de alquiler",                        "",                   "", "( cDbf )"} )
...
Function aAntFacCli()
   ...
   aAdd( aColFacCli, { "nNumFac",   "N",  9,   0, "",                   "",   "",   "( cDbfCol )" } )
   aAdd( aColFacCli, { "cSufFac",   "C",  2,   0, "",                   "",   "",   "( cDbfCol )" } )
   aAdd( aColFacCli, { "cSerAnt",   "C",  1,   0, "",                   "",   "",   "( cDbfCol )" } )
   aAdd( aColFacCli, { "nNumAnt",   "N",  9,   0, "",                   "",   "",   "( cDbfCol )" } )
   aAdd( aColFacCli, { "cSufAnt",   "C",  2,   0, "",                   "",   "",   "( cDbfCol )" } )
   aAdd( aColFacCli, { "cDivAnt",   "C",  3, 0, "Código de la divisa",  "",   "",   "( cDbfCol )" } )
   aAdd( aColFacCli, { "nVdvAnt",   "N", 10, 4, "Cambio de la divisa",  "",   "",   "( cDbfCol )" } )
   aAdd( aColFacCli, { "nImpAnt",   "N", 16, 6, "Importe del anticipo", "",   "",   "( cDbfCol )" } )
         ...
         if ( lExcCnt == nil                                .or.;    // Entran todos
            ...
            if ( cFacCliL )->lTotLin
               ...
               */
               ...
               nTotPnt           := nPntLFacCli( cFacCliL, nDpvDiv )
...
FUNCTION nImpLFacCli( uFacCliT, dbfFacCliL, nDec, nRou, nVdv, lIva, lDto, lPntVer, lImpTrn, cPouDiv )
   ...
   if ValType( dbfFacCliT ) == "A"

      if lIva .and. dbfFacCliL[ _NIVA ] != 0
         if !lIvaInc
            nCalculo    += nCalculo * dbfFacCliL[ _NIVA ] / 100
         end if
      else
         if lIvaInc .and. dbfFacCliL[ _NIVA ] != 0
            nCalculo    -= Round( nCalculo / ( 100 / dbfFacCliL[ _NIVA ] + 1 ), nRou )
         end if
      end if

   else

      if lIva .and. ( dbfFacCliL )->nIva != 0
         if !lIvaInc
            nCalculo    += nCalculo * ( dbfFacCliL )->nIva / 100
         end if
      else
         if lIvaInc .and. ( dbfFacCliL )->nIva != 0
            nCalculo    -= Round( nCalculo / ( 100 / ( dbfFacCliL )->nIva + 1 ), nRou )
         end if
      end if
...
STATIC FUNCTION BeginTrans( aTmp, nMode )
   ...
   */
   ...
   dbCreate( cTmpAnt, aSqlStruct( aAntFacCli() ), cLocalDriver() )
   ...
   if !NetErr()
      ...
      if ( dbfFacCliA )->( dbSeek( cFac ) )
         while ( ( dbfFacCliA )->cSerFac + Str( ( dbfFacCliA )->nNumFac ) + ( dbfFacCliA )->cSufFac == cFac ) .AND. ( dbfFacCliA )->( !eof() )
            dbPass( dbfFacCliA, dbfTmpDoc, .t. )
            ( dbfFacCliA )->( dbSkip() )
         end while
      end if
...
STATIC FUNCTION lCalcDeta( aTmp, lTotal )
   ...
   BEGIN SEQUENCE
      ...
      */
      ...
      case nMode == EDIT_MODE
         ...
         if !Empty( oStock )
            oStock:FacCli( cSerFac + Str( nNumFac ) + cSufFac, ( dbfFacCliT )->cCodAlm, .t., .t. )
         end if
         */
         ...
         */
         ...
         while ( ( dbfFacCliA )->( dbSeek( cSerFac + Str( nNumFac ) + cSufFac ) ) .and. !( dbfFacCliA )->( eof() ) )
            if dbLock( dbfFacCliA )
               ( dbfFacCliA )->( dbDelete() )
               ( dbfFacCliA )->( dbUnLock() )
            end if
      ...
      */
      ...
      ( dbfTmpAnt )->( dbGoTop() )
      while ( dbfTmpAnt )->( !eof() )
         dbPass( dbfTmpAnt, dbfFacCliA, .t., cSerFac, nNumFac, cSufFac, dFecFac )
         ( dbfTmpAnt )->( dbSkip() )
      end while
      ...
      */
      ...
      /*
      Guardamos la información para el auditor------------------------------------
      */

      #ifndef __PDA__
      if !Empty( oAuditor() )
         oAuditor():AddEventFacturaClientes( nMode, cSerFac + Str( nNumFac ) + cSufFac )
      end if
      #endif
      ...
      */
      ...
      if !Empty( oStock )
         oStock:FacCli( cSerFac + Str( nNumFac ) + cSufFac, ( dbfFacCliT )->cCodAlm, .f., .f. )
      end if
      */
      ...
      */
      ...
      #ifdef __PDA__
         if ApoloMsgNoYes( "Imprimiendo facturas.", "¿Desea continuar?" )
            pdaGenFacCli( oBrw, dbfFacCliT, dbfFacCliL )
         end if
      #endif
...
Function nImpAntCli( uFacCliA, cDbfDiv, cDivRet, lPic )
   ...
   DEFAULT uFacCliA  := dbfFacCliA
   DEFAULT cDbfDiv   := dbfDiv
   DEFAULT cDivRet   := cDivEmp()
   DEFAULT lPic      := .f.

   if ValType( uFacCliA ) == "O"
      cDivPgo        := uFacCliA:cDivAnt
      nTotRec        := uFacCliA:cImpAnt
   else
      cDivPgo        := ( uFacCliA )->cDivAnt
      nTotRec        := ( uFacCliA )->nImpAnt
   end if
