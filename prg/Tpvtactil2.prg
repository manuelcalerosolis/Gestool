...
CLASS TpvTactil
   ...
   METHOD EditFamilia()                INLINE ( if( EdtFamilia( ::aFamilias[ ::oBrwFamilias:nArrayAt, 2 ] ), ( ::CargaBrowseFamilias(), ::ChangeFamilias() ), ) )
   ...
   */
   ...
   INLINE METHOD EliminarLinea()
      ...
      WinDelRec( ::oBrwLineas, ::oTemporalLinea:cAlias, nil, nil, .f., .t. )
   ...
   */
   ...
   METHOD nUnidadesImpresasComanda()   INLINE ( ::oTemporalComanda:nImpCom )
   ...
   INLINE METHOD lEmptyAlias()
      ...
      if ( ::oTiketCabecera:nUbiTik == ubiGeneral ) .and. Empty( ::oTiketCabecera:cPntVenta ) .and. Empty( ::oTiketCabecera:cAliasTik ) .and. !Empty( ::oTemporalLinea:OrdKeyCount() )
         Return ( .t. )
      end if
   ...
   */
   ...
   INLINE METHOD ImprimeComanda( cImpresora )
      ...
      ::cImpresora      := cNombreImpresoraComanda( oUser():cCaja(), cImpresora, ::oCajaLinea:cAlias )
   ...
   INLINE METHOD SetCombinando( lCombinando )
      ...
      if ::lCombinando
         ...
         ::oBtnCombinado:ReLoadBitmap( "Cocktail_red_32" )
         ...
         ::oBtnCombinado:ReLoadBitmap( "Cocktail_32" )
...
METHOD OpenFiles() CLASS TpvTactil
   ...
   DATABASE NEW ::oCajonPortamonedas                        PATH ( cPatDat() )   FILE "CAJPORTA.DBF"        VIA ( cDriver() ) SHARED INDEX "CAJPORTA.CDX"
...
METHOD CloseFiles() CLASS TpvTactil
   ...
   if ::oCajonPortamonedas != nil .and. ::oCajonPortamonedas:Used()
      ::oCajonPortamonedas:End()
   end if
...
METHOD Resource() CLASS TpvTactil
   ...
   */
   ...
   ::oBrwFamilias:nMarqueeStyle           := MARQSTYLE_HIGHLROWMS
   ...
   */
   ...
   ::oBtnFamiliasUp     := TButtonBmp():ReDefine( 302, {|| ::oBrwFamilias:PageUp(),   ::oBrwFamilias:SelectOne() }, ::oDlg, , , .f., , , , .f., "Navigate_up2" )
   ::oBtnFamiliasDown   := TButtonBmp():ReDefine( 303, {|| ::oBrwFamilias:PageDown(), ::oBrwFamilias:SelectOne() }, ::oDlg, , , .f., , , , .f., "Navigate_down2" )
   //::oBtnFamiliasEnd    := TButtonBmp():ReDefine( 304, {|| ::oBrwFamilias:GoBottom(), ::oBrwFamilias:SelectOne() }, ::oDlg, , , .f., , , , .f., "Navigate_end_32" )

   /*
   Lista articulos favoritos---------------------------------------------------

   ::oLstFavoritos               := C5ImageView():Redefine( 201, ::oDlg )
   ::oLstFavoritos:oFont         := ::oFntBrw
   ::oLstFavoritos:nWItem        := ::nImageViewWItem
   ::oLstFavoritos:nHItem        := ::nImageViewHItem
   ::oLstFavoritos:nVSep         := ::nImageViewVSep
   ::oLstFavoritos:nHSep         := ::nImageViewHSep
   ::oLstFavoritos:aTextMargin   := ::aImageViewTextMargin
   ::oLstFavoritos:lTitle        := ::lImagenArticulos
   ::oLstFavoritos:nHTitle       := ::nImageViewTitle
   ::oLstFavoritos:lShowOption   := .f.
   ::oLstFavoritos:lxVScroll     := .f.
   ::oLstFavoritos:lxHScroll     := .f.
   ::oLstFavoritos:lAdjust       := .f.
   ::oLstFavoritos:nClrTextSel   := CLR_BLACK
   ::oLstFavoritos:nClrPane      := CLR_WHITE
   ::oLstFavoritos:nClrPaneSel   := CLR_WHITE
   ::oLstFavoritos:nAlignText    := nOr( DT_TOP, DT_CENTER, DT_WORDBREAK )

   ::oLstFavoritos:nOption       := 0
   ::oLstFavoritos:bAction       := {|| ::SeleccionaFavoritos() }
   */
   ...
   */
   ...
   ::oBrwLineas:nMarqueeStyle          := MARQSTYLE_HIGHLROWMS
  ...
  if !::lImagenArticulos
   ...
   with object ( ::oBrwLineas:AddCol() )
      :cHeader                := "Detalle"
      :bEditValue             := {|| if( Empty( ::oTemporalLinea:cComent ), Rtrim( ::oTemporalLinea:cNomTil ) + if( !Empty( ::oTemporalLinea:cNcmTil ), " con " + CRLF + ::oTemporalLinea:cNcmTil, "" ), "[*] " + Rtrim( ::oTemporalLinea:cNomTil ) + if( !Empty( ::oTemporalLinea:cNcmTil ), " con " + ::oTemporalLinea:cNcmTil, "" )) }
      :nWidth                 := 168
   end with
   ...
   */
   ...
   ::oBtnLineasTop            := TButtonBmp():ReDefine( 120, {|| ::oBrwLineas:GoUp(), ::oBrwLineas:SelectOne() },    ::oDlg, , , .f., , , , .f., "Navigate_up" )
   ::oBtnLineasBottom         := TButtonBmp():ReDefine( 121, {|| ::oBrwLineas:GoDown(), ::oBrwLineas:SelectOne() },  ::oDlg, , , .f., , , , .f., "Navigate_down" )
...
METHOD StartResource() CLASS TpvTactil
   ...
   if Empty( ::oOfficeBar )
      ...
      ::oGrpSalones        := TDotNetGroup():New( oCarpeta, 186, "Salones", .f., , "Cup_32" )
         ...
         ::oBtnLlevar      := TDotNetButton():New( 60, ::oGrpSalones, "Wheel_32",               "Para llevar",       3, {|| ::OnClickParaLlevar() }, , , .f., .f., .f. )
      ...
      oGrupo               := TDotNetGroup():New( oCarpeta, 126, "Tickets", .f., , "Index_32" )
         oBoton            := TDotNetButton():New( 60, oGrupo, "Tickets_pendientes_32",         "Pendientes",        1, {|| ::OnClickPendientes() }, , , .f., .f., .f. )
   ...
   do case
      ...
      */
      ...
      case cKey == "*"
         ...
         if !Empty( ::cGetUnidades )
            if ::oTemporalLinea:ordKeyCount()  != 0
               ::oTemporalLinea:nUntTil := Val( ::cGetUnidades )
            end if
         ::cGetUnidades     := ""
...
METHOD CreateTemporal() CLASS TpvTactil
   ...
   */
   ...
   DEFINE DATABASE ::oTemporalLinea FILE ( ::cTemporalLinea ) CLASS "TikL" ALIAS "TikL" PATH ( cPatTmp() ) VIA ( cLocalDriver() ) COMMENT "Lineas ticket cliente"
      ...
      INDEX TO ( ::cTemporalLinea ) TAG ( Str( ::oTemporalLinea:Recno() ) ) ON ( Str( ::oTemporalLinea:Recno() ) ) COMMENT "Orden" NODELETED OF ::oTemporalLinea
   ...
   */
   ...
   if ::oArticulo:Seek( cCodigoArticulo )
      ...
      if !::lCombinando
         ...
         if !Empty( ::oArticulo:cFacCnv )
            ::oTemporalLinea:nFcmCnv   := oRetFld( ::oArticulo:cFacCnv, ::oFactoresConversion, "nFacCnv" )
         end if
...
METHOD SeleccionarDefecto( cComentarioFamilia, oBrwLineasComentarios, oBrwComentarios ) CLASS TpvTactil
   ...
   if !Empty( cComentarioFamilia )
      ...
      if ::oComentariosCabecera:Seek( cComentarioFamilia )
...
METHOD SelecionaCliente() CLASS TpvTactil
   ...
   local cCliente                   := BrwCliTactil( nil, nil, nil, .t., ::oTiketCabecera:nUbiTik == ubiLlevar )
...
METHOD OnClickSalaVenta() CLASS TpvTactil
      ...
      */
      ...
      if ::GetUsuario()
         ...
         */
         ...
         if ::oRestaurante:Sala( nil, .t. ) //  nil, .f. )
...
function CheckRes()
   ...
   for n = 1 to Len( aResources )
      if aResources[ n, 2 ] != 0
         cInfo = aResources[ n, 1 ] + "," + AllTrim( Str( aResources[ n, 2 ] ) ) + ;
                  "," + aResources[ n, 3 ] + CRLF
