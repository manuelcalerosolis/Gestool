   ...
   */
   ...
   if !Empty( oAuditor() )
      if nDevice == IS_PRINTER
         oAuditor():AddEvent( PRINT_ALBARAN_CLIENTES,    cAlbaran, ALB_CLI )
      else
         oAuditor():AddEvent( PREVIEW_ALBARAN_CLIENTES,  cAlbaran, ALB_CLI )
      end if
   end if
...
FUNCTION AlbCli( oMenuItem, oWnd, cCodCli, cCodArt, cCodPed )
   ...
   */
   ...
   DEFINE SHELL oWndBrw FROM 0, 0 TO 22, 80 ;
      ...
      ALIAS    ( dbfAlbCliT );
      APPEND   ( WinAppRec( oWndBrw:oBrw, bEdtRec, dbfAlbCliT, cCodCli, cCodArt, cCodPed ) );
      DUPLICAT ( WinDupRec( oWndBrw:oBrw, bEdtRec, dbfAlbCliT, cCodCli, cCodArt, cCodPed ) );
      EDIT     ( WinEdtRec( oWndBrw:oBrw, bEdtRec, dbfAlbCliT, cCodCli, cCodArt, cCodPed ) );
   ...
   */
      ...
      */
      ...
      REDEFINE SAY oSayTxtDias ;
		...
		REDEFINE GET aGet[_CCODSUALB] VAR aTmp[_CCODSUALB];
         ...
         /*
         Comentado para Gorka
         VALID    IsCodSuAlb( aTmp[_CCODSUALB], dbfAlbCliT, nMode );
         */
   ...
   ACTIVATE DIALOG   oDlg ;
      ON INIT        (  EdtRecMenu( aTmp, oDlg ), ;
                        ...
                        oBrwInc:Load() ,;
                        oBrwPgo:Load() ,;
                        if( !Empty( cCodPed ), aGet[ _CNUMPED ]:lValid(), ) );
...
STATIC FUNCTION GrpPed( aGet, aTmp, oBrw )
   ...
   */
   ...
   DEFINE DIALOG  oDlg ;
      ...
      TITLE       "Cliente : "   + RTrim( aTmp[_CNOMCLI] ) + Space(1) + ;
                  "Obra : "      + if( Empty( aTmp[_CCODOBR] ), "TODAS", Rtrim( aTmp[_CCODOBR] ) ) + Space(1) + ;
                  "Pedidos: "    + if( aTmp[ _LIVAINC ], "IGIC incluido", "IGIC desglosado" )
...
FUNCTION nPreAlbCli( dbfAlbCliL, uTmp, nDec, lIva )
   ...
   local nDtoUno
   local nDtoDos
   local nCalculo := ( dbfAlbCliL )->NPREUNIT
...
#ifdef __PDA__
   if ApoloMsgNoYes( "Imprimiendo albaranes.", "¿Desea continuar?" )
      pdaGenAlbCli( oBrw, dbfAlbCliT, dbfAlbCliL )
   end if
#endif
