...
STATIC FUNCTION EdtRec( aTmp, aGet, dbfCajT, oBrw, bWhen, bValid, nMode )
   ...
   REDEFINE FOLDER oFld ;
      ...
      */
      ...
      TBtnBmp():ReDefine( 331, "gc_cut_16",,,,,{|| PrintEscCode( aTmp[ ( dbfCajT )->( FieldPos( "cCodCut" ) ) ], aTmp[ ( dbfCajT )->( FieldPos( "cWinTik" ) ) ] ) }, oFld:aDialogs[2], .f., , .f., "Test de c�digo" )
...
Static Function SavRec( aTmp, aGet, dbfCajT, oBrw, oDlg, nMode )
   ...
   if nMode == APPD_MODE .or. nMode == DUPL_MODE
      ...
      if dbSeekInOrd( aTmp[ ( dbfCajT )->( FieldPos( "cCodCaj" ) ) ], "CCODCAJ", dbfCajT )
         MsgStop( "C�digo ya existe " + Rtrim( aTmp[ ( dbfCajT )->( FieldPos( "cCodCaj" ) ) ] ) )
         return nil
      end if
   ...
   if empty( aTmp[ ( dbfCajT )->( FieldPos( "cNomCaj" ) ) ] )
      MsgStop( "Nombre de caja no puede estar vac�o" )
   ...
   //Le ponemos el nuevo c�digo en el caso de duplicar-------------------------
   ...
   // Asignaci�n a la variable de texto----------------------------------------
...
STATIC FUNCTION EdtDet( aTmp, aGet, dbfTmpLin, oBrw, bWhen, bValid, nMode, aTmpCaj )
   ...
   DEFINE DIALOG oDlg RESOURCE "lCajas" TITLE LblTitle( nMode ) + "impresoras de comandas"
      ...
      TBtnBmp():ReDefine( 131, "gc_cut_16",,,,,{|| PrintEscCode( aTmp[ ( dbfTmpLin )->( FieldPos( "cCodCut" ) ) ], aTmp[ ( dbfTmpLin )->( FieldPos( "cNomPrn" ) ) ] ) }, oDlg, .f., , .f., "Test de c�digo" )
...
STATIC FUNCTION EndDetalle( aTmp, aGet, dbfTmpLin, oBrw, oDlg, nMode, aTmpCaj )
   ...
   if empty( aTmp[ ( dbfTmpLin )->( FieldPos( "cTipImp" ) ) ] )
      cErrors     += "* El tipo de impresora no puede estar vac�o" + CRLF
   end if

   if empty( aTmp[ ( dbfTmpLin )->( FieldPos( "cNomPrn" ) ) ] )
      cErrors     += "* El nombre de impresora no puede estar vac�o" + CRLF
   end if
   ...
   /*
   Rellenamos el campo con el c�digo de la caja-------------------------------
   */
...
FUNCTION cCajas( oGet, dbfCajT, oGet2 )
   ...
   if ( dbfCajT )->( dbSeek( xValor ) )
      ...
      msgStop( "C�digo de caja no encontrada" )
...
FUNCTION BrwCajas( oGet, oGet2, lBigStyle )
   ...
   if lBigStyle
      ...
      REDEFINE BUTTON ;
         ID       110 ;
         OF       oDlg ;
         ACTION   ( ( dbfCajT )->( OrdSetFocus( "cCodCaj" ) ), oBrw:Refresh(), oDlg:cTitle( "Seleccionar cajas ordenado por: c�digo" ) )
...
Function BrwCajaTactil( oGet, dbfCaja, oGet2, lReturnCaja, lParaLlevar )
   ...
   if GetSysMetrics( 1 ) == 560
      ...
      DEFINE DIALOG oDlg RESOURCE "HelpEntryTactilCli_1024x576" TITLE "Seleccionar cliente ordenado por: Tel�fono"
      ...
      REDEFINE BUTTON ;
         ID       120 ;
         OF       oDlg ;
         ACTION   ( ( dbfCaja )->( OrdSetFocus( "cCodCaj" ) ), oBrw:Refresh(), oDlg:cTitle( "Seleccionar caja ordenada por: c�digo" ) )
...
Static Function ValidCajaPadre( aGet, aTmp )
   ...
   if ( dbfCajT )->( dbSeekInOrd( aTmp[ ( dbfCajT )->( FieldPos( "cCajPrt" ) ) ], "cCodCaj", dbfCajT ) )  
      ...
      if aTmp[ ( dbfCajT )->( FieldPos( "cCajPrt" ) ) ] == aTmp[ ( dbfCajT )->( FieldPos( "cCodCaj" ) ) ]
         ...
         msgStop( "C�digo de caja padre no puede ser la misma." )
      ...
      msgStop( "C�digo de caja no existe." )
...
Function aItmCaja()
   ...
   aAdd( aBase, { "cCodCaj",   "C",  3,   0, "C�digo de la caja" } )
   aAdd( aBase, { "cNomCaj",   "C", 30,   0, "Nombre de la caja" } )
   aAdd( aBase, { "cAlmCaj",   "C",  3,   0, "Almac�n de la caja" } )
   aAdd( aBase, { "cLocCaj",   "C", 50,   0, "" } )
   aAdd( aBase, { "nUsrCaj",   "N",  3,   0, "" } )
   aAdd( aBase, { "lPrnTik",   "L",  1,   0, "L�gico impresora normal tickets" } )
   aAdd( aBase, { "cPrnTik",   "C",  3,   0, "Formato tickets" } )
   aAdd( aBase, { "lPrnAlb",   "L",  1,   0, "L�gico impresora normal albaranes" } )
   aAdd( aBase, { "cPrnAlb",   "C",  3,   0, "Formato albaranes" } )
   aAdd( aBase, { "lPrnFac",   "L",  1,   0, "L�gico impresora normal facturas" } )
   aAdd( aBase, { "cPrnFac",   "C",  3,   0, "Formatos facturas" } )
   aAdd( aBase, { "cCapCaj",   "C",  3,   0, "C�digo captura" } )
   aAdd( aBase, { "lPrnVal",   "L",  1,   0, "L�gico impresora normal vales" } )
   aAdd( aBase, { "cPrnVal",   "C",  3,   0, "Formato vales" } )
   aAdd( aBase, { "lPrnDev",   "L",  1,   0, "L�gico impresora normal devoluciones" } )
   ...
   aAdd( aBase, { "cCodPrn",   "C",  3,   0, "C�digo impresora de tickets" } )
   aAdd( aBase, { "cCodVis",   "C",  3,   0, "C�digo visor" } )
   aAdd( aBase, { "cCajon",    "C",  3,   0, "C�digo caj�n portamonedas" } )
   aAdd( aBase, { "lPrnArq",   "L",  1,   0, "L�gico impresora normal arqueos" } )
   ...
   aAdd( aBase, { "lPrnEnt",   "L",  1,   0, "L�gico impresora normal entrega" } )
   aAdd( aBase, { "cPrnEnt",   "C",  3,   0, "formato de entrega" } )
   aAdd( aBase, { "cWinTik",   "C",  250, 0, "Impresora de tickets" } )
   aAdd( aBase, { "lPrnPgo",   "L",  1,   0, "L�gico impresora normal metapagos" } )
   ...
   aAdd( aBase, { "cCodBal",   "C",  3,   0, "C�digo de la balanza" } )
   aAdd( aBase, { "cCodCut",   "C",  120, 0, "C�digo de corte de papel" } )
   ...
   aAdd( aBase, { "lPrnEna",   "L",  1,   0, "L�gico impresora normal entregas a cuenta de albaranes" } )
   ...
   aAdd( aBase, { "lNoArq",    "L",  1,   0, "L�gico para no incluir en arqueo" } )
   aAdd( aBase, { "lPrnCie",   "L",  1,   0, "L�gico impresora normal arqueos ciegos" } )
   ...
   aAdd( aBase, { "cNumTur",   "C",  6,   0, "N�mero del turno" } )
...
//---------------------------------------------------------------------------//
/*
Devuelve el c�digo de la impresora de tickets que usa esta caja
*/
...
//---------------------------------------------------------------------------//
// Funci�n que chequea la caja y nos deja pasar
...
Function SelectCajas()
   ...
   if oDlg:nResult == IDOK
      ...
      msgInfo( "No selecciono ninguna caja, se establecer� la caja por defecto." + CRLF + ;
...
Function cNumeroSesionCaja( cCodCaj, dbfCaja, dbfTurno )
   ...
   if dbSeekInOrd( cCodCaj, "cCodCaj", dbfCaja )
      ...
      msgStop( "C�digo de caja " + cCodCaj + " no encontrada." )
