...
FUNCTION TpvDelRec()
   ...
   */
   ...
   //if !lQuestion .or. oUser():lNotConfirmDelete() .or. MsgNoYes( "¿Desea eliminar definitivamente este registro?", "Confirme supersión" )
...
Static Function StartEdtRec( aTmp, aGet, nMode, oDlgTpv, oBrw, oBrwDet, aNumDoc, cCodArt )
   ...
   if Empty( oOfficeBar )
      ...
      oGrupo            := TDotNetGroup():New( oCarpeta, 376, "Cobros", .f. )
         ...
         oBtnDev        := TDotNetButton():New( 60, oGrupo, "Cashier_Delete_32",        "Devolución o vale",   6, {|| AsistenteDevolucionTiket( aTmp, aGet, nMode ) }, , {|| nMode == APPD_MODE }, .f., .f., .f. )
...
Static Function TmpTiket( aTmp, aGet, nMode, lClean, lImprimirComanda )
   ...
   */
   ...
   if !Empty( oSalaVentas )                         .and.;
      ...
      AllTrim( aTmp[ _CPNTVENTA ] ) == "Genérico"   .and.;
   ...
   if lNotVta .or. lMsgVta
      ...
      if ( nStockActual - aTmp[ _NUNTTIL ] ) < 0
         ...
         if lMsgVta
            ...
            SetLostFocusOff()
            lOk   := MsgYesNo( "No hay stock suficiente, tenemos " + Alltrim( Trans( nStockActual, MasUnd() ) ) + " unidad(es) disponible(s) en almacén " + aTik[ _CALMTIK ] + ".", "¿Continuar con la venta?" )
            SetLostFocusOn()
...
Static Function lValidDlgTpv( aTmp, aGet, nSaveMode )
   ...
   if ( dbfTmpL )->( LastRec() ) == 0  .or.;
      MsgYesNo( "¿Desea guardar el ticket antes de salir?", "Selecciona una opción" )
...
Static Function StartEdtBig( aTmp, aGet, oDlgTpv, oBrwDet, nMode )
   ...
   if Empty( oOfficeBar )
      ...
      oGrupo               := TDotNetGroup():New( oCarpeta, 66, "Salones", .f. )
         oBtnTarifa        := TDotNetButton():New( 60, oGrupo, "Cup_32",                   "Genérico",         1, {|| GetSalaVenta( aTmp, aGet ) }, , , .f., .f., .f. )
   ...
   if GuardaVenta( aTmp, aGet )
      ...
      */
      ...
      if Empty( oSalaVentas:cSelectedPunto )
         oSalaVentas:cSelectedPunto  := "Genérico"
      end if
...
Static Function nGenTikCli( nDevice, cCaption, cCodDoc, cPrinter )
   ...
   While nImpYet < nCopTik
      ...
      if nImpYet < 1 .or. msgYesNo( "¿Desea imprimir el tiket Nº" + Str( nImpYet + 1, 2 ) + "?", "Elija una opción" )
...
Static Function VariableReport( oFr )
   ...
   */
   ...
   oFr:AddVariable(     "Lineas de tickets",   "Total IVA línea de factura",                    "CallHbFunc('nIvaLTpv')" )
   ...
   oFr:AddVariable(     "Lineas de tickets",   "Detalle del artículo para comanda",             "CallHbFunc('cTpvDesCmd')" )
      ...
      case nMode == EDIT_MODE
         ...
         if dbSeekInOrd( cNumTik, "cNumTik", dbfTikT )
            ...
            if !pdaIsLockSemaphore( dbfTikT )
               ...
               msgInfo( "El ticket está actualmente bloqueado", cNumTik )
               lOpen       := .f.
            end if
         ...
         case IsTrue( oSalaVentas:lPuntosVenta )
            ...
            if IsFalse( lPuntosLibres )
               ...
               if !Empty( oSalaVentas:cSelected() )
                  ...
                  if MsgYesNo( "¿Desea cambiar la ubicación actual a la sala " + Alltrim( oSalaVentas:cSelectedSala ) + ", " + Alltrim( oSalaVentas:cSelectedPunto ) + " ?" )
...
function aItmTik()
   ...
   aAdd( aItmTik , { "cTurVal",  "C",      6,     0, "Turno de la liquidación del vale" }                      )
...
function aColTik()
   ...
   aAdd( aColTik, { "cComent",  "C",    254,     0, "Comentario para el artículo",        "",                  "", "( cDbfCol )" } )
