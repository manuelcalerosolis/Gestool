...
CLASS TInfTikoStkG FROM TInfGen
...
METHOD Create()
   ...
   ::AddGroup( {|| ::oDbf:cCodGrp }, {|| "Grupo Família : " + Rtrim( ::oDbf:cCodGrp ) + "-" + oRetFld( ::oDbf:cCodGrp, ::oGruFam:oDbf ) }, {||"Total grupo família..."} )
   ...
   while ::oDbfAlm:cCodAlm >= ::cAlmOrg .AND. ::oDbfAlm:cCodAlm <= ::cAlmDes .AND. !::oDbfAlm:Eof()
      ...
      if ::oDbfArt:Seek( ::cArtOrg, .t. )
         ...
         if ::oDbfArt:NCTLSTOCK == 1                                               .AND.;
            cCodGruFam( ::oDbfArt:Codigo, ::oDbfArt, ::oDbfFam ) >= ::cGruFamOrg   .AND.;
...
METHOD PrnTiket( lPrev )
   ...
   */
   ...
   if empty( cPort )
      ...
      if oPrnTik  == nil
         oPrnTik     := TImpresoraTiket():New():lBuildComm()
     end if
   end if
   ...
   oPrn:Write( PadC( Rtrim( ::cTitle ), 40 )       + CRLF )
   oPrn:Write( PadC( Rtrim( ::cSubTitle ), 40 )    + CRLF )
   oPrn:Write( CRLF )
   ...
   while !::oDbf:Eof()
      if ::oDbf:cCodAlm != cCodAlm
         oPrn:Write( "Almacen : " + ::oDbf:cCodAlm + CRLF )
         oPrn:Write( Replicate( "-", 40 ) + CRLF )
         oPrn:Write( Padr( "Código", 8 ) + Space( 1 ) + Padr( "Artículo", 20 ) + Space( 1 ) + Padl( "Unidades", 10 ) + CRLF )
         oPrn:Write( Replicate( "-", 40 ) + CRLF )
         cCodAlm  := ::oDbf:cCodAlm
      end if
      oPrn:Write( left( ::oDbf:cCodArt, 8 ) + Space( 1 ) + Padr( ::oDbf:cDesArt, 20 ) + Space( 1 ) + Trans( ::oDbf:nNumUnd, Right( MasUnd(), 10 ) ) + CRLF )
      ::oDbf:Skip()
   end while
