...
METHOD DetCreateFields()
   ...
   ::AddField( "cNomGrp", "C", 20, 0, {|| "@!" },        "Grupo",                      .f., "Nom. grupo",                  20, .f. )
...
METHOD AcuCreate()
   ...
   ::AddField( "cGrpFam", "C",  3, 0, {|| "@!" },           "Grp. grupo",     .t., "Cod. grupo"        ,  5, .f. )
   ::AddField( "cNomGrp", "C", 20, 0, {|| "@!" },           "Grupo",          .t., "Nom. grupo"        , 20, .f. )
...
//---------------------------------------------------------------------------//
 ...
 METHOD AppPre( lAcumula )
   ...
   ::oDbf:cNomGrp       := oRetFld( ::oPreCliL:cGrpFam, ::oGruFam:oDbf )
...
METHOD AppPed( lAcumula )
   ...
   ::oDbf:cNomGrp       := oRetFld( ::oPedCliL:cGrpFam, ::oGruFam:oDbf )
...
METHOD AddRAlb()
   ...
   */
   ...
   nTotImpUni           := nImpLAlbCli( ::oAlbCliT:cAlias, ::oAlbCliL:cAlias, ::nDecOut, ::nDerOut, ::nValDiv )
   nImpDtoAtp           := nDtoAtpAlbCli( ::oAlbCliT:cAlias, ::oAlbCliL:cAlias, ::nDecOut, ::nDerOut, ::nValDiv )
...
METHOD AddRFac()
   ...
   */
   ...
   nTotImpUni           := nImpLFacCli( ::oFacCliT:cAlias, ::oFacCliL:cAlias, ::nDecOut, ::nDerOut, ::nValDiv  )
   nImpDtoAtp           := nDtoAtpFacCli( ::oFacCliT:cAlias, ::oFacCliL:cAlias, ::nDecOut, ::nDerOut, ::nValDiv )
...
METHOD AddRFacRec()
   ...
   */
   ...
   nTotImpUni           := -( nImpLFacRec( ::oFacRecT:cAlias, ::oFacRecL:cAlias, ::nDecOut, ::nDerOut, ::nValDiv ) )
   nImpDtoAtp           := 0
...
METHOD IncluyeCero()
   ...
   */
   ...
   ::oGruFam:oDbf:GoTop()
   while !::oGruFam:oDbf:Eof()

      if ( ::lAllGrp .or. ( ::oGruFam:oDbf:cCodGrp >= ::cGruFamOrg .AND. ::oGruFam:oDbf:cCodGrp <= ::cGruFamDes ) ) .AND.;
         !::oDbf:Seek( ::oGruFam:oDbf:cCodGrp )

         ::oDbf:Append()
         ::oDbf:Blank()
         ::oDbf:cGrpFam    := ::oGruFam:oDbf:cCodGrp
         ::oDbf:cNomGrp    := ::oGruFam:oDbf:cNomGrp
         ::oDbf:Save()
...
METHOD IncluyeCeroArt()
   ...
   ::oMtrInf:SetTotal( ::oGruFam:oDbf:OrdKeyCount() )

   while !::oGruFam:oDbf:Eof()

      if ( ::lAllGrp .or. ( ::oGruFam:oDbf:cCodGrp >= ::cGruFamOrg .AND. ::oGruFam:oDbf:cCodGrp <= ::cGruFamDes ) )

         while !::oDbfArt:Eof()

            if ( ::lAllArt .or. ( ::oDbfArt:Codigo >= ::cArtOrg .AND. ::oDbfArt:Codigo <= ::cArtDes ) ) .and.;
               !::oDbf:Seek( ::oGruFam:oDbf:cCodGrp + ::oDbfArt:Codigo )
      ...
      ::oMtrInf:AutoInc( ::oGruFam:oDbf:OrdKeyNo() )

      ::oDbfArt:GoTop()

   end while

   ::oMtrInf:AutoInc( ::oGruFam:oDbf:LastRec() )
...
METHOD NewGroup( lDesPrp )
   ...
   if lDesPrp
      ::AddGroup( {|| ::oDbf:cGrpFam + ::oDbf:cCodArt + ::oDbf:cCodPr1 + ::oDbf:cCodPr2 + ::oDbf:cValPr1 + ::oDbf:cValPr2 + ::oDbf:cLote },;
