...
CLASS TShell FROM TMdiChild
   ...
   METHOD FastSeek( nKey, nFlags, oGet )
...
METHOD New(  nTop, nLeft, nBottom, nRight, cTitle, oMenu, oWnd, oIcon,;
   ...
   */
   ...
   if oWndBar() != nil
      ...
      ::oBtnBar            := TTreeView():New( 42, 0, Self, Rgb( 0, 0, 0 ), Rgb( 255, 255, 255 ), .t., .f., 140, 400 )

      if ::lBigStyle
         ::oBtnBar:SetItemHeight( 40 )
      else
   ...
   do case
   case Valtype( xAlias ) == "O"
      ::oBrw               := TWBrowse():New( 42, 145, ::nRight - ::nLeft, ::nBottom - ::nTop, bLine, aHeaders, aColSizes, Self,,,,,,, ::oFont,,,,, .f., ::xAlias:cAlias, .t., , .f. )
      xAlias:SetBrowse( ::oBrw, .f. )

   case ValType( xAlias ) == "C"
      ::oBrw               := TWBrowse():New( 42, 145, ::nRight - ::nLeft, ::nBottom - ::nTop, bLine, aHeaders, aColSizes, Self,,,,,,, ::oFont,,,,, .f., ::xAlias, .t.,, .f. )
   ...
   case xAlias == nil
      ::oBrw               := TWBrowse():New( 42, 145, ::nRight - ::nLeft, ::nBottom - ::nTop - 32 - 25, bLine, aHeaders, aColSizes, Self,,,,,,, ::oFont,,,,, .f., ::xAlias, .t., , .f. )
   ...
   @ 37, 0  SPLITTER             ::oHorizontalSplitter ;
            HORIZONTAL ;
            PREVIOUS CONTROLS    ::oBtnTop ;
            HINDS CONTROLS       ::oBtnBar, ::oBrw ;
            LEFT MARGIN          0 ;
            RIGHT MARGIN         0 ;
            SIZE 800, 4          PIXEL ;
            OF                   Self ;
   ...
   @ 41,140 SPLITTER             ::oVerticalSplitter ;
            VERTICAL ;
            PREVIOUS CONTROLS    ::oBtnBar ;
            HINDS CONTROLS       ::oBrw ;
            LEFT MARGIN          0 ;
            RIGHT MARGIN         0 ;
            SIZE 4, 800          PIXEL ;
...
METHOD Search() CLASS TShell
   ...
   REDEFINE COMBOBOX ::oTxtSea VAR xCadena;
      ...
      ::oTxtSea:oGet:bChange  := {| nKey, nFlags | ::FastSeek( nKey, nFlags ) }
...
METHOD FastSeek( nKey, nFlags, oGet ) CLASS TShell
...
METHOD AddSeaBar() CLASS TShell
   ...
   if oWndBar() != nil
      ...
      oWndBar():SetGetChange(       {| nKey, nFlags | ::FastSeek( nKey, nFlags, oWndBar():oGet ) } )
      ...
      ::oTxtSea:bKeyDown   := {| nKey, nFlags | ::KeySearch( nKey ) }

      @  0, 0 COMBOBOX ::oComboBox ;
         VAR      ::cComboBox ;
         OF       ::oBtnBar ;
         ITEMS    ::aPrompt ;
         SIZE     120, 100 ;
...
Method AddImageList( cImage )
   ...
   local nImageList  := nil

   if !Empty( cImage )
      ::oImageList:AddMasked( TBitmap():Define( cImage ), Rgb( 255, 0, 255 ) )
...
Method ClickTree()
   ...
   oItem       := ::oBtnBar:GetSelected()

   if oItem != nil .and. Valtype( oItem:Cargo ) == "B"
      Eval( oItem:Cargo )
   end if
