...
METHOD AnuRutFields()
   ...
   ::AddField ( "cCodRut", "C",  4, 0, {|| "@!" },         "Cod. Rut.",       .t., "Código ruta",             4 )
   ::AddField ( "cNomRut", "C", 50, 0, {|| "@!" },         "Ruta",            .t., "Nombre ruta",            20 )
...
METHOD AcuCreate()
   ...
   ::AddField( "cCodRut", "C",  4, 0, {|| "@!" },           "Cod. Rut.",      .t., "Código ruta"       ,  4, .f. )
   ::AddField( "cNomRut", "C", 50, 0, {|| "@!" },           "Ruta",           .t., "Nombre ruta"       , 20, .f. )
...
METHOD AddPre( lAcumula )
   ...
   if !lAcumula .or. !::oDbf:Seek( ::oPreCliT:cCodRut )
...
METHOD AddPed( lAcumula )
   ...
   if !lAcumula .or. !::oDbf:Seek( ::oPedCliT:cCodRut )
...
METHOD AddAlb( lAcumula )
   ...
   if !lAcumula .or. !::oDbf:Seek( ::oAlbCliT:cCodRut )
...
METHOD AddFac( lAcumula )
   ...
   if !lAcumula .or. !::oDbf:Seek( ::oFacCliT:cCodRut )
...
METHOD AddFacRec( lAcumula )
   ...
   if !lAcumula .or. !::oDbf:Seek( ::oFacRecT:cCodRut )
...
METHOD AddTik( cCodArt, nPrecio, lAcumula )
   ...
   if !lAcumula .or. !::oDbf:Seek( ::oTikCliT:cCodRut )
   ...
   */
   ...
   while !::oDbfRut:Eof()
      ...
      if ( ::lAllRut .or. ( ::oDbfRut:cCodRut >= ::cRutOrg .AND. ::oDbfRut:cCodRut <= ::cRutDes ) ) .AND.;
         !::oDbf:Seek( ::oDbfRut:cCodRut )

         ::oDbf:Append()
         ::oDbf:Blank()
         ::oDbf:cCodRut    := ::oDbfRut:cCodRut
         ::oDbf:cNomRut    := ::oDbfRut:cDesRut
         ::oDbf:Save()
...
METHOD NewGroup( lDesPrp )
   ...
   if lDesPrp
      ::AddGroup( {|| ::oDbf:cCodRut + ::oDbf:cCodArt + ::oDbf:cCodPr1 + ::oDbf:cCodPr2 + ::oDbf:cValPr1 + ::oDbf:cValPr2 + ::oDbf:cLote },;
