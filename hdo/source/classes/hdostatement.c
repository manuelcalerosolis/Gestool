...
HB_METHOD( TSTMT_BINDCOLUMN )
{
	...
	if( stmt )
	{
		HDO_RET_SUCCEED( STMT_BINDCOLUMN( stmt, hb_param( 1, HB_IT_NUMINT | HB_IT_STRING ),
...
HB_METHOD( TSTMT_BINDPARAM )
{
	...
	if( stmt )
	{
		HDO_RET_SUCCEED( STMT_BINDPARAM( stmt, hb_param( 1, HB_IT_NUMINT | HB_IT_STRING ),
...
HB_METHOD( TSTMT_BINDVALUE )
{
	...
	if( stmt )
	{
		HDO_RET_SUCCEED( STMT_BINDVALUE( stmt, hb_param( 1, HB_IT_NUMINT | HB_IT_STRING ), hb_param( 2, HB_IT_ANY ) ) );
	}
	else
	{
		hb_retl( HB_FALSE );
