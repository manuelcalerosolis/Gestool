...
HB_METHOD( THDO_NEW )
{
	...
	if( hdo )
	{
		/* Carga los metodos en la estructura HDO */
		hdo_setMethod( hdo, hb_parc( 1 ), hb_parclen( 1 ) );
		/* Ejecuta el metodo init de la RDL especifica */
		HDO_INIT( hdo );
		/* MUY IMPORTANTE: Asigna la estructura recien creada a la clase */
		hb_objSetPtr( pSelf, IVAR_HDO, hdo );
	}
	else
	{
		hdo_throwErrOut( HDO_NOTMEMHDO, NULL );
	}
...
HB_METHOD( THDO_CONNECT )
{
	...
	if( hdo )
	{
		HDO_RET_SUCCEED( HDO_CONNECT( hdo, hb_parc( 1 ), hb_parc( 2 ),
									  hb_parc( 3 ), hb_parc( 4 ),
									  hb_parnint( 5 ), hb_parc( 6 ),
									  hb_parnl( 7 ) ) );
	}
	else
	{
...
HB_METHOD( THDO_DISCONNECT )
{
	...
	if( hdo )
	{
		HDO_RET_SUCCEED( HDO_DISCONNECT( hdo ) );
	}
	else
	{
		hb_retl( HB_FALSE );
...
/***
 * Metodo: EXEC
 * Uso: Ejecuta una sentencia que no devuelve un conjunto de regisatros ej.: insert, update
...
HB_METHOD( THDO_EXEC )
{
	...
	if( hdo )
	{
		...
		if( pStmt )
		{
			HDO_EXEC( hdo, hb_itemGetCPtr( pStmt ), hb_itemGetCLen( pStmt ), &ulRes );
		}
	}
...
{
	PHDO hdo = hb_getHDO();
	PHB_ITEM oStmt = hb_itemNew( NULL );
	...
	if( hdo )
	{
		...
		if( pStmt )
		{
			HDO_PREPARE( hdo, hb_itemGetCPtr( pStmt ), hb_itemGetCLen( pStmt ), oStmt );
		}
	}
...
HB_METHOD( THDO_QUERY )
{
	...
	if( hdo )
	{
		...
		if( pStmt )
		{
			HDO_QUERY( hdo, hb_itemGetCPtr( pStmt ), hb_itemGetCLen( pStmt ), oStmt );
		}
	}
...
HB_METHOD( THDO_BEGINTRANSACTION )
{
	...
	if( hdo )
	{
		HDO_RET_SUCCEED( HDO_BEGINTRANSACTION( hdo ) );
	}
	else
	{
		hb_retl( HB_FALSE );
...
HB_METHOD( THDO_INTRANSACTION )
{
	...
	if( hdo )
	{
		HDO_RET_SUCCEED( HDO_INTRANSACTION( hdo ) );
	}
	else
	{
		hb_retl( HB_FALSE );
...
HB_METHOD( THDO_COMMIT )
{
	...
	if( hdo )
	{
		HDO_RET_SUCCEED( HDO_COMMIT( hdo ) );
	}
	else
	{
		hb_retl( HB_FALSE );
...
HB_METHOD( THDO_ROLLBACK )
{
	...
	if( hdo )
	{
		HDO_RET_SUCCEED( HDO_ROLLBACK( hdo ) );
	}
	else
	{
		hb_retl( HB_FALSE );
...
HB_METHOD( THDO_SETATTRIBUTE )
{
	...
	if( hdo )
	{
		HDO_RET_SUCCEED( HDO_SETATTRIBUTE( hdo, hb_parni( 1 ), hb_param( 2, HB_IT_ANY ) ) );
	}
	else
	{
		hb_retl( HB_FALSE );
