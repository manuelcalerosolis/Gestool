...
HB_METHOD( TROWSET_NEW )
{
	...
	if( rs )
	{
		...
		if( oStmt )
		{
			PSTMT stmt = ( PSTMT ) hb_arrayGetPtr( oStmt, POS_PSTMT );
			
			if( setMethodRS( rs, stmt ) != HB_SUCCESS )
			{
				hdo_throwErrNew( HDO_NOTMEMRS, NULL );
			}
			
			STMT_COLUMNCOUNT( stmt, &rs->uiColCount );
			RS_INIT( rs );

			hb_arraySetPtr( pSelf, IVAR_RS, rs );
		}
		else
		{
			DimeC( "TODO: Error de argumento. No hay objeto TStmt" );
		}
	}
	else
	{
...
HB_METHOD( TROWSET_GOTO )
{
	...
	if( rs )
	{
		...
		{
			DimeC( "TODO: Error de falta de argumento" );
		}
	}
}
...
HB_METHOD( TROWSET_FINDNEXT )
{
	...
	if( rs )
	{
		...
		if( pFind )
		{
			...
			if( !fEof )
			{
				RS_FIND( rs, pFind, pxCol, HB_FALSE, &ulRec );	
			}
