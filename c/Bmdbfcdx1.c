/*
 * $Id: bmdbfcdx1.c,v 1.34 2007/09/25 07:32:33 marchuet Exp $
 */
...
static ERRCODE hb_cdxSeekWild( CDXAREAP pArea, BOOL fSoftSeek, PHB_ITEM pKeyItm, BOOL fFindLast, BOOL fNext, BOOL bAll )
{
   ...
   else
   {
      ...
      if( ! bAll && pArea->lpdbPendingRel && pArea->lpdbPendingRel->isScoped )
         SELF_FORCEREL( ( AREAP ) pArea );
...
HB_FUNC( BM_DBSETFILTERARRAY )
{
    ...
    if( pArea )
    {
       ...
       hb_errRT_DBCMD( EG_NOTABLE, EDBCMD_NOTABLE, NULL, "BM_DBSETFILTERARRAY" );
}
...
HB_FUNC( BM_DBSETFILTERARRAYADD )
{
    ...
    if( pArea && pArea->dbfi.fOptimized )
    {
       ...
       hb_errRT_DBCMD( EG_NOTABLE, EDBCMD_NOTABLE, NULL, "BM_DBSETFILTERARRAYADD" );
}
...
HB_FUNC( BM_DBSETFILTERARRAYDEL )
{
    ...
    if( pArea && pArea->dbfi.fOptimized )
    {
       ...
       hb_errRT_DBCMD( EG_NOTABLE, EDBCMD_NOTABLE, NULL, "BM_DBSETFILTERARRAYDEL" );
}
    ...
    if( pArea )
    {
       if( !ISNIL( 1 ) )
       {
          ...
          if ( bAll)
          {
             ...
             if( hb_cdxSeekWild( (CDXAREAP) pArea, bSoftSeek, pKey, bFindLast, bNext, bAll ) == SUCCESS )
             {
          ...
          hb_errRT_DBCMD( EG_ARG, EDBCMD_SEEK_BADPARAMETER, NULL, "BM_DBSEEKWILD" );
    }
    else
       hb_errRT_DBCMD( EG_NOTABLE, EDBCMD_NOTABLE, NULL, "BM_DBSEEKWILD" );
...
{
    if ( SUPER_PUTREC( (AREAP) pArea, pBuffer ) == SUCCESS )
    {
        if ( pArea->dbfi.fFilter && pArea->dbfi.fOptimized )
        {
            pArea->dbfi.fFilter = FALSE;
...
static ERRCODE hb_cdxOrderInfo( CDXAREAP pArea, USHORT uiIndex, LPDBORDERINFO pInfo )
{
   ...
   {
      case DBOI_CONDITION:
         pInfo->itmResult = hb_itemPutC( pInfo->itmResult, ( pTag ? pTag->ForExpr : "" ) );
      ...
      case DBOI_EXPRESSION:
         pInfo->itmResult = hb_itemPutC( pInfo->itmResult, pTag ? pTag->KeyExpr : "" );
         break;
      ...
      case DBOI_NAME:
         pInfo->itmResult = hb_itemPutC( pInfo->itmResult, pTag ? pTag->szName : "" );
         break;
      ...
      case DBOI_FULLPATH:
         pInfo->itmResult = hb_itemPutC( pInfo->itmResult, pTag ? pTag->pIndex->szFileName : "" );
         break;
