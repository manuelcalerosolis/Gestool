...
{                           // nClrText, nClrTextOver, nTop, nLeft, nHeight, nWidth, nPad
    ...
    hOldFont                = ( HFONT ) SelectObject( hDC, bIsOver && !bIsOverMenu ? hFontOver : hFont );
...
HB_FUNC( CLEANWB )      // hWnd, hDC, nWidth, nHeight, nLeft, nTop, hPalBmpBk, nColor
{
    ...
    if( hPalBmpBk != 0 )
    {
        ...
        hBmpOld             = ( HBITMAP ) SelectObject( hDCMemX, hPalBmpBk );
...
HB_FUNC( COMPABMP )     // ( hDC, nWidth, nHeight )
{
