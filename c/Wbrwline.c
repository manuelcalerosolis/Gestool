...
static void FW_DrawText( HDC hDC, RECT * rct, LPCSTR pText,
                         ...
                         BOOL bHeadFoot, BOOL bBkOnly ) ; // CeSoTech
...
static void near PaintTheLine( HDC hDC, RECT * rct, WORD wIndex,
                               ...
                               BOOL bDrawFocusRect,
							   PCLIPVAR pPrevPaint, PCLIPVAR pNextPaint ) ;
...
static void near PaintTheLine( HDC hDC, RECT * rct, WORD wIndex,
                               ...
                               LONG nClrLine, BOOL bFooter,
                               BOOL bSelect, PCLIPVAR pFont,
                               BOOL bDrawFocusRect, PCLIPVAR pPrevPaint, PCLIPVAR pNextPaint )
{
   ...
   while( wIndex <= wLen )
   {
        ...
        {                                     //(Es lo mismo no hacer esto,
           ...
           else  // Si es Numerico Bmp no Tree, o , es Character !!!!
           {
               ...
               {
                  // BLOQUE PREVIO DE PINTADO VITEH !!!
                  bPainted = TRUE ;
				  bBkOnly  = FALSE ;
                  if( pPrevPaint ) // Bloque de pintado personalizado de celda
                  {
                     _PutSym( _SymEval );
                     _xPushM( pPrevPaint );
                     _PutLN( wRowPos );
                     ...
                     _PutLN( bFooter ? 2 : ( bHeader ? 1 : ( bSelect ? 3 : 0 ) ) );
                     _PutLN( (LONG) hDC );
                     _PutCL( (LPSTR) ( wType & CHARACTER ) ? cValue : "",
						     strlen(( wType & CHARACTER ) ? cValue : "")  );
                     _PutLN( (LONG) rct->top );
                     _PutLN( (LONG) rct->left );
				     _PutLN( (LONG) rct->bottom );
				     _PutLN( (LONG) rct->right );
                     _xEval( 9 ) ;
                     if ( ( _parinfo( -1 ) & NUMERIC ) && ( _parnl( - 1 ) == 0 ) )
                        bPainted = FALSE ;
					 if ( ( _parinfo( -1 ) & NUMERIC ) && ( _parnl( - 1 ) == 1 ) )
                        bBkOnly = TRUE ;
                  }

				  // PINTADO DE CELDA ESTANDARD !!!
				  if ( bPainted )
					  FW_DrawText( hDC, rct,
                                  ( wType & CHARACTER ) ? cValue : "",
                                  wAlign, wcLen, hFont, bHeader, bBkOnly ) ;

                  // BLOQUE POSTERIOR DE PINTADO VITEH !!!
                  if( pNextPaint ) // Bloque de pintado personalizado de celda
                  {
                     _PutSym( _SymEval );
                     _xPushM( pNextPaint );
                     _PutLN( wRowPos );
...
CLIPPER WBRWLINE( PARAMS ) // ( hWnd, hDC, nRow, aText, aSizes, nFirstItem, ;
                           ...
                           // bFont, lDrawFocusRect, bPrevPaint, bNextPaint )
#endif

{
   ...
   if ( ! bFooter )
   {
      ...
      rct.top    = rct.bottom - (wFooterHeight+1)    +2;
   }

   rct.left   = 0;

   PaintTheLine( hDC, &rct, _parni( 6 ), hb_param( 4, -1 ), hb_param( 5, -1 ),
                 hWhitePen, hGrayPen, bTree,
                 ISARRAY(11) ? hb_param( 11, -1 ) : 0, _parni( 12 ),
                 (wRow == 0), _parni( 13 ),
                 _parni( 14 ), _parl( 15 ),
				 ...
				 ISBLOCK( 23 ) ? hb_param( 23, -1 ) : 0,   // CeSoTech
				 ISBLOCK( 24 ) ? hb_param( 24, -1 ) : 0 ); // CeSoTech
...
CLIPPER WBRWPANE( PARAMS ) // ( hWnd, hDC, Self, bLine, aSizes, nFirstItem,
                           ...
                           //   bTextColor, bBkColor, nClrLine, nColorFondo, bFont,
						   //   bPrevPaint, bNextPaint ) // New's by CesoTech
#endif
{
   HWND hWnd        = ( HWND ) _parnl( 1 );
   HDC hDC          = ( HDC ) _parnl( 2 );
   ...
   while( wRow <= wRows && wSkipped == 1 )
   {
      ...
      rct.right  = client.right;
         ...
         PaintTheLine( hDC, &rct, wIndex, _tos,
                       ...
                       FALSE,
                       ISBLOCK( 19 ) ? hb_param( 19, -1 ) : 0,   // CeSoTech
					   ISBLOCK( 20 ) ? hb_param( 20, -1 ) : 0 )  // CeSoTech
...
static void FW_DrawText( HDC hDC, RECT * rct, LPCSTR pText,
                         ...
                         HFONT hFont, BOOL bHeadFoot, BOOL bBkOnly )
...
#ifdef __HARBOUR__
HARBOUR HB_FUN_TWSAVESCREEN()
#else
CLIPPER TWSAVESCRE( PARAMS ) //EN ( hWnd ) --> { cPRECT, hBmp }
#endif
{
   HWND hWnd        = ( HWND ) _parnl( 1 );
   HDC hDC          = GetDC( hWnd ) ;
   ...
   hBmp    = CreateCompatibleBitmap( hDC, rct.right - rct.left, rct.bottom - rct.top );
   hBmpOld = SelectObject( hDC2, hBmp );
   BitBlt( hDC2, 0, 0, rct.right, rct.bottom, hDC, 0, 0, SRCCOPY ); // Copia al BMP
   ...
   hb_reta( 2 );
   _storc( (LPSTR) &rct, -1, 1 );
   _stornl( (LONG) hBmp, -1, 2 );
...
#ifdef __HARBOUR__
HARBOUR HB_FUN_TWRESTSCREEN()
#else
CLIPPER TWRESTSCRE( PARAMS ) // EN ( hWnd, hDC, cPRECT, hBmp )
#endif
{
   HDC hDC          = ( HDC ) _parnl( 2 ) ;
   HDC hDC2 ;
   ...
   hBmp = ( HBITMAP ) _parnl( 4 );

   hDC2    = CreateCompatibleDC( hDC );
   hBmpOld = SelectObject( hDC2, hBmp );
   BitBlt( hDC, 0, 0, rct->right, rct->bottom, hDC2, 0, 0, SRCCOPY );
...
#ifdef __HARBOUR__
HARBOUR HB_FUN_TWHEADERACTION()
#else
CLIPPER TWHEADERAC( PARAMS ) // TION  ( nAction, hWnd, nCol, cCaption, nWidth, nAlign, hBmp )
#endif                       //        nAction Values--> 1 Insert, 2 Modifify, 3 Delete, 4 return Width
{
    HDITEM hdi;
 	LONG lJustify = HDF_LEFT;

    _bset( ( char * ) &hdi, 0, sizeof( hdi ) );

	if ( _parni( 7 ) )
        hdi.mask = HDI_TEXT | HDI_FORMAT | HDI_WIDTH | HDI_BITMAP ;
    else
        hdi.mask = HDI_TEXT | HDI_FORMAT | HDI_WIDTH ;

    if ( _parni( 1 ) == 3 )  {
       _retl( Header_DeleteItem( ( HWND ) _parnl( 2 ), _parni( 3 )-1 ) );
       return ;
    }
    if ( _parni( 1 ) == 4 )  {
       Header_GetItem( ( HWND ) _parnl( 2 ), _parni( 3 )-1, (LPARAM) &hdi ) ;
       _retni( hdi.cxy );
       return ;
    }
    ...
    if (_parni(6) & HA_CENTER)
       lJustify = HDF_CENTER ;

	if (_parni(6) & HA_RIGHT)
       lJustify = HDF_RIGHT ;

    hdi.fmt        = lJustify | HDF_STRING ;
    hdi.hbm        = (HBITMAP) _parni( 7 );

    if ( _parni( 1 ) == 1 )
       _retl( Header_InsertItem(( HWND ) _parnl( 2 ), _parni( 3 )-1, (LPARAM) &hdi ) );

    if ( _parni( 1 ) == 2 )
       _retl( Header_SetItem( ( HWND ) _parnl( 2 ), _parni( 3 )-1, (LPARAM) &hdi ) );
...
#ifdef __HARBOUR__
HARBOUR HB_FUN_TWHEADERNOTIFYPOS()
#else
CLIPPER TWHEADERNO( PARAMS ) // TIFYPOS( nPtrNMHDR ) --> nPos
#endif
{
   NMHEADER * pNMHDR = ( NMHEADER * ) _parnl( 1 );

   _retnl( pNMHDR->iItem + 1);
}
