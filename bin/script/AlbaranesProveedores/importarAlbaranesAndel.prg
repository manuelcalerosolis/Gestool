...
STATIC FUNCTION IcgAlbPrv( aFichero, oDlg, oInforme )
   ...
   */
   ...
   oInforme:cText( strtran( strtran( cInforme, "</p>", CRLF ), "<p>", "" ) )
...
Static Function IcgDetAlbPrv( cSerDoc, cSufDoc, cDesLin, nUntLin, nPvpLin, nDtoLin, cRefLin )
   ...
   if !dbSeekInOrd( cRefLin, "Codigo", D():Articulos( nView ) )
      ...
      cInforme                += "<p>" + "Articulo " + cRefLin + ", " + cDesLin + " no existe en la base de datos" + "</p>" 
      cInforme                += "<p>" + "Albaran número " + cSerDoc + "/" + Alltrim( Str( nNumAlb ) ) + "/" + RetSufEmp() + "</p>"

      lIncidencia             := .t.
   else 
      if ( Round( ( D():Articulos( nView ) )->pCosto, 2 ) != ( Round( ( nPvpLin ) - ( nPvpLin * nDtoLin / 100 ), 2 ) ) )
         cInforme             += "<p>" + "Articulo " + cRefLin + ", " + cDesLin + " ha variado su precio de costo," + "</p>"
         cInforme             += "<p>" + "Familia " + ( D():Articulos( nView ) )->Familia + ", " + alltrim( retFamilia( ( D():Articulos( nView ) )->Familia, D():Familias( nView ) ) ) + "</p>"
         cInforme             += "<p>" + "Precio nuevo " + alltrim( str( round( ( nPvpLin ) - ( nPvpLin * nDtoLin / 100 ), 2 ) ) ) + "</p>"
         cInforme             += "<p>" + "Precio anterior " + alltrim( str( round( ( D():Articulos( nView ) )->pCosto, 2 ) ) ) + "</p>"
         lIncidencia          := .t.
      end if
   end if
