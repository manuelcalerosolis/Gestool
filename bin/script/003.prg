...
static function ImportaFactura( cFichero )
   ...
   */
   ...
   cFile                            := cGetFile( "Excel ( *.Xlsx ) | " + "*.Xlsx", "Seleccione la hoja de calculo" )
      ...
      for n := nLineaComienzo to 65536
         ...
         */
         ...
         if !Empty( oOleExcel:oExcel:ActiveSheet:Range( "A" + lTrim( Str( n ) ) ):Value )
            ...
            nPesoKg                    := oOleExcel:oExcel:ActiveSheet:Range( "B" + lTrim( Str( n ) ) ):Value  
            nPesoKg                    := nGetNumeric( nPesoKg )

            Comment                    := oOleExcel:oExcel:ActiveSheet:Range( "L" + lTrim( Str( n ) ) ):Value  
   
            if ( dbfArticulo )->( dbSeek( cCodArt ) )
   
               ( dbfArticulo )->( dbRLock() )
               ( dbfArticulo )->nPesoKg  := nPesoKg
               ( dbfArticulo )->mComent  := Comment
