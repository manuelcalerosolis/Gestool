...
Function Luncher()
   ...
   MsgRun(  "Proceso de exportacion a fichero " + cFile, "Espere por favor...", {|| ExportaStock() } )
...
Function ExportaStock()
   ...
   if lOpenFiles()
         ...
         if ( dbfArticulo )->lPubInt
            ...
            if nTotStockAct != 0
               ...
               fWrite( hFile, AllTrim( ( dbfArticulo )->Codigo ) + ";" + ;
                              ...
                              AllTrim( Trans( ( dbfArticulo )->pVenta1, "@E 999,999,999.99" ) ) + ;
                              Chr( 13 ) + Chr( 10 ) )
      ...
      oInt           := TInternet():New()
      oFtp           := TFtp():New( "ftp.arguellesipo.com", oInt, "demonio@arguellesipo.com", "demoniopass1.", .f. )
