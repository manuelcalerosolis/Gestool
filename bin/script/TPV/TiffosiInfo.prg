...
METHOD processInformation() CLASS TiffosiInformation
   ...
   while ( D():Tikets( ::nView ) )->dFecTik <= ::dFin .and. ( D():Tikets( ::nView ) )->( !eof() )
      ...
      if ( D():TiketsLineas( ::nView ) )->( dbSeek( D():TiketsId( ::nView ) ) )
         ...
         while ( D():TiketsId( ::nView ) == D():TiketsLineasId( ::nView ) ) .and. ;
               ( D():TiketsLineas( ::nView ) )->( !eof() ) 

            if ( nTotNTpv( D():TiketsLineas( ::nView ) ) ) > 0 .and.;
               ( D():Articulos( ::nView ) )->( dbSeek( ( D():TiketsLineas( ::nView ) )->cCbaTil ) )

               if alltrim( ( D():Articulos( ::nView ) )->cCodFab ) == "006" 
...
METHOD writeClient()
   ...
   ::oExcel:oExcel:WorkSheets( 1 ):Cells( 2, 6 ):Value := "Abucoco Market S.L. - Tiffosi adulto"
