...
Function Inicio()
   ...
   local oTiffosiInformation
   local cOld              := HB_LangSelect( "EN" )

   oTiffosiInformation     := TiffosiInformation():New()
...
CREATE CLASS TiffosiInformation
...
METHOD New() CLASS TiffosiInformation
...
METHOD Dialog() CLASS TiffosiInformation
...
METHOD OpenFiles() CLASS TiffosiInformation
...
METHOD processInformation() CLASS TiffosiInformation
      ...
      if ( D():TiketsLineas( ::nView ) )->( dbSeek( D():TiketsId( ::nView ) ) )
         ...
         while ( D():TiketsId( ::nView ) == D():TiketsLineasId( ::nView ) ) .and. ( D():TiketsLineas( ::nView ) )->( !eof() ) 
            ...
            if ( nTotNTpv( D():TiketsLineas( ::nView ) ) ) > 0 .and. ( D():Articulos( ::nView ) )->( dbSeek( ( D():TiketsLineas( ::nView ) )->cCbaTil ) )
               ...
               if alltrim( ( D():Articulos( ::nView ) )->cCodFab ) == "007" 
...
METHOD writeClient()
   ...
   ::oExcel:oExcel:WorkSheets( 1 ):Cells( 2, 6 ):Value := "Abucoco Market S.L. - Tiffosi kids"
