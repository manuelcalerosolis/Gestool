...
CREATE CLASS ImportacionUVERisi
   ...
   DATA aDitributors                            INIT { "90 - Blanco", "91 - Begines", "92 - Dismaga" }
...
METHOD processLineUVE( cLine )
   ...
   if !( hb_isarray( aLine ) )
      msgalert( "La linea no se puede convertir en array." )
      Return ( Self )
   end if 

   if !( len( aLine ) >= 30 )
      msgalert( hb_valtoexp( aline ), "la linea no contiene los campos necesarios")
      Return ( Self )
   end if 
