...
CLASS ArticulosICG
   ...
   DATA nRow
   DATA nLineaComienzo     INIT  7

   DATA cCodigoArticulo    INIT  ""
   DATA cNombreArticulo    INIT  ""
   DATA nCostoArticulo     INIT  0
   ...
   METHOD getPropiedadesArticulos( cColumnCosto )
...
METHOD ProcessFile( cFichero ) CLASS ArticulosICG
   ...
   CursorWait()
      ...
      ::getPropiedadesArticulos( "P", "L" )

      // Segunda hoja----------------------------------------------------------

      ::oOleExcel:oExcel:WorkSheets( 2 ):Activate() // Hojas de la hoja de calculo

      // Recorremos la hoja de calculo--------------------------------------------

      SysRefresh()

      ::getPropiedadesArticulos( "T", "K" )
...
METHOD SetArticulo()
   ...
   ( D():Articulos( ::nView ) )->Nombre      := ::cNombreArticulo
   ( D():Articulos( ::nView ) )->pCosto      := ::nCostoArticulo
   ( D():Articulos( ::nView ) )->TipoIVA     := "G"

   if !empty( ::nPrecio1    )
      ( D():Articulos( ::nView ) )->pVenta1  := ::nPrecio1
   end if 
   if !empty( ::nPrecio1IVA )
      ( D():Articulos( ::nView ) )->pVtaIva1 := ::nPrecio1IVA
   end if 
   if !empty( ::nPrecio2    )
      ( D():Articulos( ::nView ) )->pVenta2  := ::nPrecio2
   end if 
   if !empty( ::nPrecio2IVA )
      ( D():Articulos( ::nView ) )->pVtaIva2 := ::nPrecio2IVA
   end if 
   if !empty( ::nPrecio3    )
      ( D():Articulos( ::nView ) )->pVenta3  := ::nPrecio3
   end if 
   if !empty( ::nPrecio3IVA )
      ( D():Articulos( ::nView ) )->pVtaIva3 := ::nPrecio3IVA
   end if 
   if !empty( ::nPrecio4    )
      ( D():Articulos( ::nView ) )->pVenta4  := ::nPrecio4
   end if 
   if !empty( ::nPrecio4IVA )
      ( D():Articulos( ::nView ) )->pVtaIva4 := ::nPrecio4IVA
   end if 
...
METHOD getPropiedadesArticulos( cColumnCosto, cColumnTarifa )
   ...
   for ::nRow := ::nLineaComienzo to 65536
      ...
      */
      ...
      ::cCodigoArticulo                := ::GetRange( "D" )
      ::cNombreArticulo                := ::GetRange( "E" ) 
      if !empty( ::GetRange( "G" ) )
         ::cNombreArticulo             += space( 1 ) + ::GetRange( "G" ) 
      end if 
      ::nCostoArticulo                 := ::GetNumeric( cColumnCosto )

      nCapacidad                       := ::GetNumeric( "G" )
      nTarifa                          := ::GetNumeric( cColumnTarifa )

      if Valtype( nCapacidad ) == "N" .and. Valtype( nTarifa ) == "N"
         ::nPrecio1                    := nCapacidad * nTarifa
         ::nPrecio1IVA                 := ::nPrecio1 + ( ::nPrecio1 * 21 / 100 )

         ::nPrecio2                    := ::nPrecio1 + ( ::nPrecio1 * 9 / 100 )
         ::nPrecio2IVA                 := ::nPrecio2 + ( ::nPrecio2 * 21 / 100 )

         ::nPrecio3                    := ::nPrecio1 + ( ::nPrecio1 * 12.5 / 100 )
         ::nPrecio3IVA                 := ::nPrecio3 + ( ::nPrecio3 * 21 / 100 )

         ::nPrecio4                    := ::nPrecio1 + ( ::nPrecio1 * 12.5 / 100 )
         ::nPrecio4IVA                 := ::nPrecio4 + ( ::nPrecio4 * 21 / 100 )
      else 
         ::nPrecio1                    := nil
         ::nPrecio1IVA                 := nil
         ::nPrecio2                    := nil
