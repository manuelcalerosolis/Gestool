...
METHOD Create()
   ...
   ::AddField( "cGrpFam", "C",  3, 0, {|| "@!" },        "Cod. grupo",                 .t., "Cod. grupo",                 5 )
   ::AddField( "cNomGrp", "C", 20, 0, {|| "@!" },        "Nom. grupo",                 .t., "Nom. grupo",                20 )
...
METHOD OpenFiles() CLASS TRnkGVta
...
METHOD CloseFiles() CLASS TRnkGVta
...
METHOD lResource( cFld ) CLASS TRnkGVta

   if !::StdResource( "RNKVTAGRP" )
      return .f.
   end if
...
METHOD lGenerate() CLASS TRnkGVta
   ...
   if !::lAllGrp
      cExpLine       += ' .and. cGrpFam >= "' + ::cGruFamOrg + '" .and. cGrpFam <= "' + ::cGruFamDes + '"'
   end if
